// i18n.js - Complete Bilingual Database (English & 中文)
// The Adult Piano Companion: A Definitive Guide & Tool Matrix for Self-Directed Learning

const I18N = {
  currentLang: "en",
  secondaryLang: null, // null or 'zh' / 'en' for side-by-side comparison

  ui: {
    en: {
      siteBadge: "🎹 Self-Directed Adult Piano Guide",
      siteTitle: "The Adult Piano Companion",
      siteSubtitle: "A definitive guide and open tool matrix for adult learners — grounded in personal agency, deliberate practice, and the living philosophy of self-directed mastery.",
      blogSubtitleLink: "Not a ToE",
      blogSubtitleSuffix: "read the essay: The Unobservable Driver of Learning.",
      
      navPhilosophy: "🌱 Philosophy & Ground",
      navMatrix: "🧰 Tool Matrix",
      navComparator: "⚖️ Trade-off Comparator",
      navPractice: "🧠 Practice Architecture",
      navRepertoire: "🎼 Repertoire Ladder",
      navMatcher: "🧭 Toolkit Matcher",
      navFavorites: "❤️ Resources I Enjoy",

      searchPlaceholder: "Search by tool, book, app, author, technique, or keyword...",
      filterAllCategories: "All Categories",
      catApps: "Interactive Apps",
      catScores: "Sheet Music & Readers",
      catEarTheory: "Ear Training & Theory",
      catYouTube: "YouTube Mentors",
      catBooks: "Method Books & Literature",
      catHardware: "Hardware & Ergonomics",

      filterLevelAll: "All Stages",
      filterLevelBeginner: "Absolute Beginner",
      filterLevelIntermediate: "Intermediate",
      filterLevelAdvanced: "Advanced",

      filterCostAll: "Any Cost",
      filterCostFree: "Free / Open",
      filterCostPaid: "Paid / Sub",

      resultsCount: "Showing {count} resources",
      noResults: "No matching tools found. Try clearing filters or changing search keywords.",
      viewDetails: "View Details & Breakdown →",
      closeModal: "Close",
      keyStrengths: "Key Strengths & Value",
      criticalCaveats: "Limitations & Misconceptions to Avoid",
      bestSynergy: "Recommended Practice Synergy",
      officialLink: "Visit Resource / Channel",

      comparatorTitle: "Critical Trade-offs in Adult Piano Learning",
      comparatorSubtitle: "Every method or technology is scaffolding. Compare approaches honestly to choose the right lever for your current stage.",
      
      practiceTitle: "Adult Neuroplasticity & Practice Architecture",
      practiceSubtitle: "Adult brains don't lack plasticity; they lack unexamined repetition. Discover how deliberate, low-tension loops wire deep muscle memory.",

      repertoireTitle: "Curated Adult Repertoire Ladder",
      repertoireSubtitle: "From early coordination to expressive mastery. Real pieces that build technique without patronizing children's nursery rhymes.",

      matcherTitle: "Interactive Toolkit Matcher",
      matcherSubtitle: "Answer 4 questions about your musical goals, available time, and setup to generate your custom practice blueprint.",
      btnStartQuiz: "Start Assessment",
      btnRestartQuiz: "Retake Assessment",
      quizResultHeading: "Your Personalized Learning Blueprint",

      favoritesTitle: "Resources I Enjoy & Aesthetic Inspirations",
      favoritesSubtitle: "Beyond technical drills lies the true wellspring of music: the artists, arrangements, and channels that sustain genuine aesthetic joy and fuel daily deliberate practice.",
      favoritesWhyEnjoy: "Why I Deeply Enjoy This",
      favoritesHighlights: "Signature Highlights & Works",
      favoritesTakeaway: "Insight for Adult Learners",
      favoritesVisit: "Watch Channel / Explore ↗",

      footerQuote: "“Ability is the accumulated loop of chosen relation, not a fixed endowment. Tools remain secondary scaffolding; learning is the non-transferable physical friction (+1) generated from within.”",
      footerNote: "The Adult Piano Companion • An open, independent research project on adult self-directed musicianship."
    },
    zh: {
      siteBadge: "🎹 成人钢琴自主研习指南",
      siteTitle: "成人钢琴研习指南与工具全景",
      siteSubtitle: "专为成年人打造的自主学琴指南与开放工具矩阵——扎根于第一人称心智原点、刻意练习与自我探索，打破年龄迷思与方法神话。",
      blogSubtitleLink: "Not a ToE",
      blogSubtitleSuffix: "参阅深度博文：《学习中不可观测的引擎》。",

      navPhilosophy: "🌱 核心心法与原点",
      navMatrix: "🧰 工具与资源矩阵",
      navComparator: "⚖️ 路径与工具对照",
      navPractice: "🧠 神经重塑与练习架构",
      navRepertoire: "🎼 成人进阶曲目阶梯",
      navMatcher: "🧭 个性化方案匹配",
      navFavorites: "❤️ 挚爱资源与灵感",

      searchPlaceholder: "搜索工具、软件、图书、导师、曲目或技能关键词...",
      filterAllCategories: "全部类别",
      catApps: "互动软件与App",
      catScores: "乐谱库与阅读器",
      catEarTheory: "练耳、乐理与节拍",
      catYouTube: "YouTube顶尖导师",
      catBooks: "教材、练习曲与专著",
      catHardware: "硬件设备与人机工学",

      filterLevelAll: "所有阶段",
      filterLevelBeginner: "零基础起步",
      filterLevelIntermediate: "中级进阶",
      filterLevelAdvanced: "高级挑战",

      filterCostAll: "所有费用",
      filterCostFree: "免费 / 开源",
      filterCostPaid: "付费 / 订阅",

      resultsCount: "展示 {count} 项精选资源",
      noResults: "未找到匹配的工具或资源。请尝试清空筛选或更换搜索词。",
      viewDetails: "查看深度剖析与建议 →",
      closeModal: "关闭",
      keyStrengths: "核心优势与杠杆价值",
      criticalCaveats: "局限性与警惕误区",
      bestSynergy: "推荐搭配与练习协同",
      officialLink: "访问官方网址 / 频道",

      comparatorTitle: "成人学琴关键路径与工具权衡",
      comparatorSubtitle: "任何工具与教学法都是暂时的脚手架。通过客观对比，找到适合你当下阶段的着力点。",

      practiceTitle: "成人神经可塑性与刻意练习架构",
      practiceSubtitle: "成人的大脑从不缺少神经可塑性，缺少的是带有觉察的高质量反馈回路。用低张力慢练攻克肌肉记忆。",

      repertoireTitle: "成人精选进阶曲目阶梯",
      repertoireSubtitle: "从双手协调起步，直至宏大复调与深度抒情。告别幼稚的儿歌教条，弹奏真正触动心灵的音乐作品。",

      matcherTitle: "个性化学习方案智能匹配",
      matcherSubtitle: "回答4个关于你的音乐动机、时间预算与设备现状的问题，生成量身定制的研习工具链与练习指南。",
      btnStartQuiz: "开始测试",
      btnRestartQuiz: "重新诊断",
      quizResultHeading: "你的专属成人钢琴研习蓝图",

      favoritesTitle: "挚爱资源与审美灵感",
      favoritesSubtitle: "在纯粹的技术训练之外，音乐最深处的源头始终是打动心灵的声音。这里记录了我个人深为喜爱、持续带来审美滋养与练琴动力的自学典范、音乐家与频道。",
      favoritesWhyEnjoy: "为何深爱此资源",
      favoritesHighlights: "亮点特色与代表作品",
      favoritesTakeaway: "予成年研习者的启示",
      favoritesVisit: "访问频道 / 聆听 ↗",

      footerQuote: "“能力是个体所一再选择的关系立场所积累的回路，而非某种先天的恒定禀赋。工具始终只是次生的脚手架；学习是不可外包的肉身体验与物理摩擦（+1）。”",
      footerNote: "成人钢琴研习指南 • 献给所有在键盘前探索心智主权与音乐之美的自主学习者。"
    }
  },

  // Philosophy Chapter Data
  philosophy: {
    hero: {
      en: {
        headline: "Anyone Can Learn Piano. There Is No Age Limit.",
        subheadline: "The only true prerequisite is the willingness and dedication to learn it as something you yourself want to master and enjoy.",
        quote: "“Ability is not a fixed endowment waiting to be measured; it is the accumulated loop of chosen relation. No app, teacher, or method can live or learn for you.”"
      },
      zh: {
        headline: "任何人都能学会钢琴，琴键之上从未设限年龄。",
        subheadline: "唯一真正必要的先决条件，是你内心渴望将其作为自己真正想掌握、想享受之物而投入的意愿与专注。",
        quote: "“能力不是一种等待被测量的固定禀赋，而是个体所一再选择的关系立场所积累的自强反馈回路。没有任何软件、名师或教材能替你活着，更无法替你学会。”"
      }
    },
    sections: [
      {
        id: "misconceptions",
        title: {
          en: "1. Dismantling the Dogmas & Age Misconceptions",
          zh: "1. 破除流行偏见与年龄神话"
        },
        content: {
          en: `
            <p>The prevailing narrative suggests that piano is an exclusive discipline that must be started at age four, or the window of neuroplasticity slams shut forever. This is not biology; it is cultural dogma.</p>
            <p>As an adult, your brain and fingers indeed feel different at the very beginning. Muscle memory takes conscious, deliberate repetition to etch into the motor cortex. Tendons feel stiffer; your analytical prefrontal cortex is hyper-active, constantly judging your own awkwardness and mistakes. But here is the truth:</p>
            <ul>
              <li><strong>Adults have superior harmonic and pattern intuition:</strong> You understand symmetry, voice leading, chord construction, and emotional tension intuitively through a lifetime of listening to music. A child often just presses keys by rote; an adult can hear the architecture.</li>
              <li><strong>Self-directed motivation vs. coerced compliance:</strong> A child practices because parents demand it. An adult practices because they chose to sit down at the bench. That intrinsic causal intention accelerates deep neural consolidation far beyond passive compliance.</li>
              <li><strong>Neuroplasticity never closes:</strong> Adult motor cortex rewiring simply requires higher salience and conscious deliberate loops. When you slow down and remove bodily tension, your hands will adapt with astonishing fidelity.</li>
            </ul>
          `,
          zh: `
            <p>长期以来，主流叙事总在宣扬一种决定论：钢琴必须从四岁抓起，一旦错过童年发育期，神经可塑性的窗口便会永久关闭。这根本不是生物学事实，而是体制化考级与精英教育制造的文化神话。</p>
            <p>不可否认，成人初学琴时，大脑、手指与肌肉记忆确实需要更刻意的努力去捕获。成人的指间韧带相对固定，前额叶皮层高度理性，常因即时听到的拙劣琴声而滋生自我评判与挫败感。然而，成人的优势同样巨大且无可替代：</p>
            <ul>
              <li><strong>高阶的模式识别与和声直觉：</strong> 历经数十年的阅历与音乐聆听，成人对和弦色彩、结构对仗、情绪起伏拥有深刻的直觉共鸣。孩童多半是在机械执行位置指令，而成人一眼就能识破背后的和声骨架。</li>
              <li><strong>发自主权意愿而非外力压迫：</strong> 孩子练琴往往源自父母的督促或考级压力；成年人坐在琴凳上的每一个瞬间，都源于自我灵魂深处对掌握音乐的真实渴望。这种第一人称的自主意愿，能激发极其强烈的神经递质释放，深化突触重塑。</li>
              <li><strong>成人大脑终生保持可塑性：</strong> 成人的髓鞘化虽然成熟，但神经可塑性从未终结。成人只需要学会“慢下来”、卸除全身对抗性张力，指尖的独立与灵动便会以惊人的精度生根发芽。</li>
            </ul>
          `
        }
      },
      {
        id: "personal_story",
        title: {
          en: "2. The Origin: From Accompanying My Children to Claiming My Own Voice",
          zh: "2. 个人原点：从陪娃练琴到找回自身的第一人称因果"
        },
        content: {
          en: `
            <p>I began learning piano with a very common adult excuse: I wanted to accompany my two children and support their musical education. In the opening months, progress felt agonizingly slow. My adult hands lacked flexibility; the coordination between my left and right hands felt like two stubborn minds arguing with each other. Muscle memory took dozens of repetitions to stick.</p>
            <p>Then, a quiet transformation occurred: <strong>I stopped treating piano as a tool to help someone else, and began treating it as something I myself wanted to master and enjoy.</strong></p>
            <p>Inevitably, children grow, hobbies fluctuate, and over time my kids lost interest in daily practice. But by that point, the causal driver had completely relocated. I didn't stop. I sat at the piano day after day, week after week. Today, I can play far more fluently, deeply, and expressively than my kids ever did. I learned to play pieces I once thought were reserved for conservatory prodigies — from the delicate melancholy of folk ballads like <em>Chengdu</em> to the sweeping emotion of the <em>Forrest Gump Suite</em>, the poetic minimalism of Ludovico Einaudi (whose pieces I conquered largely through Michael Maiber's patient tutorials alongside my own relentless struggling), the cinematic pulse of <em>Game of Thrones</em>, and the breathtaking architectural drama of Handel's <em>Passacaglia</em>.</p>
            <p>I tried everything on the market: popular gamified apps, waterfall video visualizers, YouTube masterclasses, classical method books, and private in-person teachers. And that experiment taught me the single most vital lesson of adult learning.</p>
          `,
          zh: `
            <p>我最初接触钢琴，起因于许多成年人家长都会经历的契机：为了陪伴我的两个孩子学琴，给他们树立榜样。刚上手时，起步确实缓慢而滞重。作为一个成年人，大脑发出指令到指尖落地总有微小的延迟，手指柔韧度不够，左右手分脑协调更是一场拉锯战，任何一个肌肉记忆都需要反复刻划才能固定。</p>
            <p>然而在摸索中，一个质的转变悄然发生：<strong>我不再把练琴仅仅当作“辅助孩子”的外在任务，而是将目光完全拉回到了学习本身——这是我内心真正渴望去驾驭、去享受的艺术世界。</strong></p>
            <p>后来，随着时间推移，孩子们的注意力不可避免地转移了，逐渐对日复一日的练习失去了热情。然而此时，学习的因果原点早已扎根在我自己体内。我没有停下。日复一日，晨昏流转，我坚持坐在琴键前。如今，我的曲目量与演奏表达早已走得比孩子们更深更远。从《成都》温润质朴的民谣织体，到《阿甘正传》主题曲的如羽毛般灵动，再到鲁多维科·艾诺迪（Ludovico Einaudi）如诗如画的极简篇章（我几乎所有艾诺迪的曲目，都是对照着自学导师 Michael Maiber 的逐句慢拆教学、再加上自己日复一日与琴键的艰辛死磕而攻克的），从《权力的游戏》的史诗律动，再到亨德尔-哈尔沃森《帕萨卡利亚》（Passacaglia）那令人窒息的宏大变奏与复调奔流——我一步步亲手攻克了这些曾以为遥不可及的经典。</p>
            <p>在这一路中，我几乎尝遍了市面上所有的路径：所有叫得出名字的流行App、下落瀑布流视频、YouTube大牌名师课、经典传统教材，以及线下一对一私教。这番亲身涉水，让我彻底看清了成人学习的唯一本质。</p>
          `
        }
      },
      {
        id: "causal_lever",
        title: {
          en: "3. The Causal Lever: Tools are Scaffolding, Never the Engine",
          zh: "3. 因果杠杆：工具皆为脚手架，唯有内部摩擦才是引擎"
        },
        content: {
          en: `
            <p>As explored in my essay <a href="https://powerpig99.github.io/not-a-toe/posts/the-unobservable-driver-of-learning/" target="_blank" rel="noopener">The Unobservable Driver of Learning</a>, humanity shares a pervasive cognitive error: <strong>we habitually allocate causal power to external factors.</strong></p>
            <p>We credit or blame the app (Simply Piano, Flowkey), the teacher, the method book, or the YouTube tutorial for our progress or lack thereof. But all external devices are secondary residue. Observation registers only downstream scores: grades, completed app levels, green checkmarks. The true generative driver — the succession of chosen relations, the tolerance for physical friction, and the relentless reflexive correction of one's own motor feedback — does not live inside the app.</p>
            <blockquote>
              <p>“Any one of these tools can help you; but none of them can help you without you making your own deliberate, internal effort.”</p>
            </blockquote>
            <p>When you use an app that scores your keystrokes via microphone or MIDI, it gives you a dopamine hit of validation. But if you rely on the screen to tell you when to play, you have outsourced your internal pulse and note-reading ear. The moment the screen turns off, the capability evaporates. In contrast, when you sit down with a pencil, deconstruct a single difficult measure, clap its rhythm, and feel the exact weight of your forearm resting into the keybed — you are paying the irreversible energetic cost (+1) of real mastery.</p>
            <p><strong>This guide itself is not a holy scripture.</strong> It is simply an organized toolkit of scaffolding. Pick up whatever tool speeds your contact with the instrument; and the moment a tool becomes a crutch that numbs your agency, set it down without hesitation.</p>
          `,
          zh: `
            <p>正如我在个人博客核心文章<a href="https://powerpig99.github.io/not-a-toe/posts/the-unobservable-driver-of-learning/" target="_blank" rel="noopener">《学习中不可观测的引擎》</a>中所论证的，人类认知中最隐蔽的陷阱，莫过于<strong>将因果解释权习惯性地让渡给外在客体</strong>。</p>
            <p>我们总以为进步或停滞取决于下载了哪款App（Simply Piano、Flowkey）、请了哪位名师、买了哪本教材，或是看了哪段YouTube教学。然而，所有这些外在载体都仅仅是次生残留物。外在观察只能捕捉到下游表象：打卡天数、软件评级、绿色的通关提示。而真正驱动神经重组的那个引擎——个体每一次主动选择的练习姿态、对真实生理摩擦的耐受度，以及对自身肌体反馈的敏锐内省——根本不在任何工具里。</p>
            <blockquote>
              <p>“市面上所有的工具都能够帮助我；但在我自身做出真实、刻意的努力之前，没有任何一个工具能独自拯救我。”</p>
            </blockquote>
            <p>当你跟着屏幕上的下落方块击键时，你获得的是即时多巴胺反馈；但如果你依赖视觉色块来决定何时按键，你便已经将内心的节奏律动与五线谱空间感彻底拱手外包。一旦屏幕熄灭，这种“熟练度”会瞬间坍塌。相反，当你拿起铅笔，拆解哪怕仅仅两小节复杂声部，亲手打出切分节奏，在缓慢的触键中感受手臂重力的自然下沉——你才是在为真正的脑神经突触生长支付不可逆的能量代价（+1）。</p>
            <p><strong>甚至包括本指南本身，也绝不是什么一锤定音的绝对真理。</strong> 它仅仅是一套经过提炼的、供你差遣的脚手架。哪样工具能加速你与琴键的亲密接触，便借用它；一旦哪样工具开始成为削弱你独立听觉与判断的拐杖，便毫不留恋地拆掉它。</p>
          `
        }
      }
    ]
  },

  // Tool Matrix Data (Curated items across 6 categories)
  tools: [
    // --- 1. Interactive Apps & Software ---
    {
      id: "piano-marvel",
      name: "Piano Marvel",
      category: "apps",
      level: "beginner",
      cost: "paid",
      platforms: ["iPad", "Mac", "Windows"],
      tags: ["Sight-Reading", "SASR", "MIDI Assessment", "Method Syllabus"],
      title: {
        en: "Piano Marvel: The Gold Standard for Sight-Reading & Structured Method",
        zh: "Piano Marvel：视奏考核（SASR）与阶梯化曲库的硬核利器"
      },
      summary: {
        en: "A serious, method-driven learning platform with the Standard Assessment of Sight Reading (SASR), graded classical syllabus, and instant MIDI timing feedback.",
        zh: "兼具专业视奏评测系统（SASR）与系统化古典教材的严肃学习软件，支持高精度MIDI音准与节拍评分。"
      },
      strengths: {
        en: "Unmatched SASR sight-reading engine that objectively measures reading fluency; breaks complex pieces into micro-slices (Prepare Mode); huge classical library from Level 1 to 18.",
        zh: "独步业界的SASR视奏考核系统，客观量化识谱敏捷度；独创“切片预备模式”（Prepare Mode）拆解难点；曲库深厚覆盖全套经典教材。"
      },
      caveats: {
        en: "UI looks somewhat utilitarian and dated; strict MIDI timing can cause robotic stiffness if the player neglects wrist relaxation and musical phrasing.",
        zh: "界面偏向传统严肃软件风格，多巴胺激励较少；若盲目追求MIDI100%时值对齐，容易忽略手腕柔韧与音乐呼吸感。"
      },
      synergy: {
        en: "Use for 10 minutes of daily SASR sight-reading drills and systematic scale/arpeggio testing, then close it and play with expression.",
        zh: "每天使用10分钟进行SASR视奏测验与音阶爬音过关，随后关闭软件回到纯粹的乐谱表达中。"
      },
      link: "https://www.pianomarvel.com/"
    },
    {
      id: "flowkey",
      name: "Flowkey",
      category: "apps",
      level: "beginner",
      cost: "paid",
      platforms: ["iOS", "Android", "Web"],
      tags: ["Dual View", "Wait Mode", "Pop & Film Repertoire"],
      title: {
        en: "Flowkey: Intuitive Dual-View Video & Interactive Sheet Follower",
        zh: "Flowkey：真人真琴俯拍与交互乐谱同步的优质助手"
      },
      summary: {
        en: "A polished app showing overhead professional hand video alongside interactive sheet music, featuring 'Wait Mode' that listens via microphone or MIDI.",
        zh: "将顶置真人琴键俯拍视频与互动乐谱上下对齐的视觉化利器，具备识别琴声停顿等待的“等待模式”。"
      },
      strengths: {
        en: "Superior UX; see real pianist hand posture and fingering while reading the score; loop selected bars easily; hands-separate practice is smooth.",
        zh: "交互设计极度优雅；在看谱的同时能直接观察示范演奏者的手型与指法转移；A-B段循环与左右手声部分离极其方便。"
      },
      caveats: {
        en: "Can encourage looking at the overhead keyboard instead of learning to read notation; microphone audio detection can occasionally mishear polyphonic bass chords.",
        zh: "容易让初学者过度依赖看上方钢琴视频模仿，削弱看谱耐心；麦克风拾音在复杂低音和弦时偶有误判，建议插MIDI线。"
      },
      synergy: {
        en: "Ideal for breaking down complex contemporary pieces (e.g. Forrest Gump, Yiruma) hands-separated during the initial learning stage.",
        zh: "在攻克电影原声、现代流行抒情曲（如《阿甘正传》《River Flows in You》）时，作为初识声部的辅助脚手架。"
      },
      link: "https://www.flowkey.com/"
    },
    {
      id: "simply-piano",
      name: "Simply Piano",
      category: "apps",
      level: "beginner",
      cost: "paid",
      platforms: ["iOS", "Android"],
      tags: ["Gamification", "Absolute Beginner", "Dopamine Hooks"],
      title: {
        en: "Simply Piano: High Initial Dopamine, Low Long-Term Autonomy",
        zh: "Simply Piano：超低起步门槛的高多巴胺游戏化试炼"
      },
      summary: {
        en: "The most popular gamified beginner app worldwide, turning basic note learning into a scroll game with backing tracks.",
        zh: "全球下载量最大的游戏化钢琴启蒙App，用伴奏音轨与卷轴通关机制将初学者带入琴键世界。"
      },
      strengths: {
        en: "Zero intimidation factor; gets adults pressing correct keys within the first 10 minutes; great for overcoming initial fear of the instrument.",
        zh: "极低的心理门槛；让从未碰过琴的成人在前10分钟内敲出正确音符并获得成就感，有效破除对琴键的畏难情绪。"
      },
      caveats: {
        en: "Dangerous 'Guitar Hero effect': students learn to react to scrolling visual cues rather than internalizing rhythm or true sheet literacy; almost no focus on wrist technique.",
        zh: "存在严重的“节奏光剑效应”：学员往往是在对滚动的光标产生条件反射，而非内化节拍与真正读谱；对落滚手腕、手臂重量一字不提。"
      },
      synergy: {
        en: "Use for the first 2-3 weeks to establish the habit of sitting at the piano daily, then immediately transition away before habits calcify.",
        zh: "仅建议在前2-3周作为建立每天坐到琴凳前习惯的催化剂，一旦习惯建立，应果断过渡至传统乐谱与严肃练习。"
      },
      link: "https://www.joytunes.com/simply-piano"
    },
    {
      id: "synthesia",
      name: "Synthesia",
      category: "apps",
      level: "beginner",
      cost: "paid",
      platforms: ["Windows", "Mac", "iPad", "Android"],
      tags: ["Waterfall Visualizer", "MIDI Import", "Spatial Patterns"],
      title: {
        en: "Synthesia: Falling-Note Waterfall Visualizer for Fast Spatial Intuition",
        zh: "Synthesia：下落瀑布流可视化与空间记忆神器"
      },
      summary: {
        en: "The classic 'waterfall' visualizer software that transforms any standard MIDI file into colorful bars tumbling toward 88 keys.",
        zh: "风靡全球的经典瀑布流练琴软件，能将任何标准MIDI文件解析为向88个琴键垂落的彩色光带。"
      },
      strengths: {
        en: "Phenomenal for understanding the physical topography and geometry of chords; lets you import any custom MIDI file from the internet; tempo manipulation is effortless.",
        zh: "极具空间直观性，能迅速看清复杂大跳与和弦跨度在键盘上的物理轮廓；支持导入网上无限量的MIDI乐曲；速度自由调节。"
      },
      caveats: {
        en: "Does not build sheet music literacy; memorizing 1,000 falling notes is exhausting compared to reading an 8-bar harmonic progression on paper.",
        zh: "完全绕开了传统记谱法；靠肉眼死记硬背几千个下落色块，不仅耗尽脑力，且离开屏幕便瞬间失忆，阻碍长期自学能力。"
      },
      synergy: {
        en: "Pair with a paper score: use Synthesia for 5 minutes solely to preview where hands cross or jump, then return to the sheet music.",
        zh: "与纸质乐谱对照使用：仅用Synthesia观察两手交叉或宽幅跳音的物理位置，一旦明确键盘方位，立刻回到乐谱本身。"
      },
      link: "https://synthesiagame.com/"
    },

    // --- 2. Sheet Music & Digital Readers ---
    {
      id: "musescore",
      name: "MuseScore & MuseScore.com",
      category: "scores",
      level: "all",
      cost: "free",
      platforms: ["Web", "Windows", "Mac", "iOS", "Android"],
      tags: ["Community Arrangements", "Transposition", "Playback Engine"],
      title: {
        en: "MuseScore: The World's Open Score Repository & Playback Companion",
        zh: "MuseScore：全球最大的开源/社区乐谱库与试听重排神器"
      },
      summary: {
        en: "A powerhouse dual platform: open-source desktop notation software (MuseScore 4) plus a massive community sheet repository with interactive audio playback.",
        zh: "开源桌面制谱软件与海量全球乐友社区乐谱库的集合体，支持网页端交互式跟随播放与一键移调。"
      },
      strengths: {
        en: "Find almost any pop song, anime theme, or film soundtrack (e.g. Chengdu, Game of Thrones, Joe Hisaishi); instantly transpose keys to suit your hands.",
        zh: "几乎能搜到任何当代影视金曲、流行动漫变奏（如《成都》《权力的游戏》《久石让》）；可随心调慢速度试听声部，支持一键移调。"
      },
      caveats: {
        en: "Quality of user-uploaded community scores varies wildly; fingerings are often missing or ergonomically awkward, requiring critical personal review.",
        zh: "社区网友自传乐谱良莠不齐，指法标注常常缺失或违背人体工学，需要研习者具备批判性甄别与自修指法的能力。"
      },
      synergy: {
        en: "When tackling a contemporary favorite, listen to multiple community arrangements on MuseScore to pick the one with the most musical voice leading.",
        zh: "选练一首喜欢的流行抒情曲目时，先在MuseScore上对比试听不同版本的织体编写，挑出和声与指法最流畅的乐谱打印。"
      },
      link: "https://musescore.com/"
    },
    {
      id: "imslp",
      name: "IMSLP / Petrucci Music Library",
      category: "scores",
      level: "all",
      cost: "free",
      platforms: ["Web", "iOS", "Android"],
      tags: ["Public Domain", "Urtext Editions", "Historical Scores"],
      title: {
        en: "IMSLP: The Virtual Library of Alexandria for Public Domain Classical Music",
        zh: "IMSLP（佩特鲁奇音乐图书馆）：古典公版乐谱的亚历山大图书馆"
      },
      summary: {
        en: "The definitive global archive of public domain sheet music, housing over 700,000 historic scores from Bach and Mozart to Handel and Chopin.",
        zh: "全人类古典音乐公版乐谱的终极数字档案馆，免费收录从巴赫、莫扎特到肖邦、亨德尔的70多万份历史名家手稿与权威乐谱。"
      },
      strengths: {
        en: "100% free and legal; compare different historical editions (Busoni, Czerny, Breitkopf, Peters) to inspect differing fingerings and articulation marks.",
        zh: "完全免费且合法；可同时下载对比不同大师编订版（如布索尼版、车尔尼版、彼得斯版），观察大师们对于同一难点的不同指法智慧。"
      },
      caveats: {
        en: "Scan qualities of older prints can be fuzzy; some 19th-century heavily edited versions add unstylistic romantic markings to baroque pieces.",
        zh: "古老版画扫描件清晰度不一；部分19世纪浪漫主义编订版擅自为巴洛克作品添加了过于厚重的踏板与强弱符号，需注意辨别。"
      },
      synergy: {
        en: "Download the pure Urtext version of Bach Inventions or Handel's Passacaglia for clean, unadulterated study notes.",
        zh: "查阅巴赫小前奏曲或亨德尔变奏时，首选IMSLP上原汁原味的Urtext（原版无修饰）PDF，亲手在谱面上做指法逻辑标记。"
      },
      link: "https://imslp.org/"
    },
    {
      id: "forscore",
      name: "forScore",
      category: "scores",
      level: "intermediate",
      cost: "paid",
      platforms: ["iPad", "Mac"],
      tags: ["Score Reader", "Apple Pencil", "Page Turner Pedal"],
      title: {
        en: "forScore: The Pro Standard Digital Sheet Music App for iPad",
        zh: "forScore：专业音乐家与深度研习者的iPad看谱终极利器"
      },
      summary: {
        en: "The indisputable benchmark iPad application for organizing, annotating, and performing from digital sheet music PDFs.",
        zh: "iPad平台上无可争议的乐谱管理与演奏标注文档神器，支持Apple Pencil高精度批注与蓝牙翻页脚踏板。"
      },
      strengths: {
        en: "Pencil latency is zero; build custom setlists; crop margins to maximize notation size; instant Bluetooth foot-pedal integration removes page-turn friction.",
        zh: "Apple Pencil手写极低延迟；可裁切乐谱冗余白边让音符最大化呈现；与蓝牙无线翻页踏板无缝结合，彻底消灭翻页手忙脚乱。"
      },
      caveats: {
        en: "iOS/iPadOS exclusive; paid one-time fee; takes an hour of onboarding to master its powerful layering and bookmarking features.",
        zh: "仅限苹果生态（iPad/Mac）；需一次性付费买断；系统功能强大但需要花一点时间熟悉图层管理与智能书签逻辑。"
      },
      synergy: {
        en: "Combine iPad + forScore + Bluetooth foot pedal for a completely clutter-free, paperless adult practice studio.",
        zh: "配备iPad、Apple Pencil与蓝牙翻页脚踏，将所有下载的公版谱或自制谱统揽其中，随时在小节上方彩色勾勒双手换指逻辑。"
      },
      link: "https://forscore.co/"
    },
    {
      id: "henle-library",
      name: "Henle Library App",
      category: "scores",
      level: "advanced",
      cost: "paid",
      platforms: ["iOS", "Android", "Mac"],
      tags: ["Urtext", "Fingerings by Masters", "Custom Layout"],
      title: {
        en: "Henle Library: Blue Urtext Precision with Switchable Master Fingerings",
        zh: "德国Henle蓝皮原版谱App：权威Urtext文本与大师指法自由切换"
      },
      summary: {
        en: "The official digital platform of G. Henle Verlag, famous for their definitive blue-bound classical Urtext scores.",
        zh: "世界顶级古典乐谱出版社德国G. Henle Verlag官方出品的数字化蓝皮谱平台，汇聚全球最严谨的学界校勘成果。"
      },
      strengths: {
        en: "Flawless typographical beauty; switch dynamically between fingerings contributed by legendary pianists (e.g. Schiff, Perahia) or erase all fingerings to write your own.",
        zh: "近乎艺术品般的印刷排版美学；独家支持在席夫、佩拉西亚等数位钢琴大师的指法方案间一键切换，甚至清空所有指法自己编排。"
      },
      caveats: {
        en: "Priced by piece or volume via digital credits; tailored primarily to classical masterworks (not pop or modern film scores).",
        zh: "采用点数购买单首或整卷乐谱，成本相对较高；专注经典古典文献，无流行或流行跨界编曲。"
      },
      synergy: {
        en: "When you are ready to conquer Bach Inventions, Chopin Preludes, or Beethoven Sonatas, invest in a Henle Urtext for lifelong authoritative study.",
        zh: "当你准备深入研读巴赫二部创意曲、肖邦前奏曲或贝多芬慢乐章时，Henle的权威版本是检验每一个音符与休止符的最可靠依据。"
      },
      link: "https://www.henle.de/en/the-henle-library-app/"
    },

    // --- 3. Ear Training, Theory & Rhythm ---
    {
      id: "functional-ear-trainer",
      name: "Functional Ear Trainer",
      category: "theory",
      level: "beginner",
      cost: "free",
      platforms: ["Web", "iOS", "Android"],
      tags: ["Alain Benbassat", "Relative Pitch", "Tonic Resolution"],
      title: {
        en: "Functional Ear Trainer: The Alain Benbassat Tonic-Resolution Method",
        zh: "Functional Ear Trainer：基于主音引力场的科学相对音高内化法"
      },
      summary: {
        en: "An acclaimed ear-training program based on the Alain Benbassat method that teaches you to recognize notes by their magnetic pull toward the tonic key center.",
        zh: "风靡国际的经典免费听力训练工具，基于Alain Benbassat主音引力法，训练耳朵感知任何音符在调性引力场中向主音回归的倾向。"
      },
      strengths: {
        en: "Superior to isolated interval training; teaches you to hear scale degrees (1 to 7) in real tonal contexts; 100% free; 5 minutes daily produces dramatic results.",
        zh: "远超传统机械的孤立音程打分法；真正建立调性中心感，让你在任何旋律中听到音级（1到7音）的稳定度与倾向；完全免费。"
      },
      caveats: {
        en: "Requires consistency over months; can feel frustrating in the first two weeks as your brain shifts from guessing to genuine harmonic feeling.",
        zh: "需要持之以恒的日常小剂量刺激；前两周当大脑从“乱猜”转向“感受引力”时会有一段迷茫期，需克服急躁心态。"
      },
      synergy: {
        en: "Do 5-7 minutes on your commute or before practice; watch your ability to transcribe songs by ear and play chords spontaneously soar.",
        zh: "每天在通勤碎片时间或正式练琴前花5-7分钟打卡，不出两月，你在琴键上“听音辩旋律”与即兴配和弦的能力将脱胎换骨。"
      },
      link: "https://www.miles.be/"
    },
    {
      id: "complete-ear-trainer",
      name: "Complete Ear Trainer",
      category: "theory",
      level: "intermediate",
      cost: "paid",
      platforms: ["iOS", "Android"],
      tags: ["Intervals", "Chord Qualities", "Inversions"],
      title: {
        en: "Complete Ear Trainer: Gamified, Rigorous Audio Architecture",
        zh: "Complete Ear Trainer：游戏化关卡机制下的硬核现代听音训练营"
      },
      summary: {
        en: "A video-game-like mastery tree that methodically trains your recognition of intervals, chord qualities (maj7, min7, dim, aug), and harmonic inversions.",
        zh: "采用技能树与星级通关制度设计的严谨视听训练App，系统化打磨大/小/减/增和弦、转位与复杂音程的辨识度。"
      },
      strengths: {
        en: "Sleek, modern UI with arcade-like progression; progressive difficulty prevents overwhelming beginners; great stats tracking.",
        zh: "现代极简界面与如同街机闯关般爽快的通关反馈；难度阶梯严密平缓，数据统计图表清晰反馈你在哪些和弦辨别上有盲区。"
      },
      caveats: {
        en: "Focuses on isolated interval identification alongside harmonic chords, which some pedagogues consider less musical than functional tonal hearing.",
        zh: "包含较多无调性背景的孤立音程辨识，需与具有真实调性上下文的曲目实践相结合，防范“为了应试而听音”。"
      },
      synergy: {
        en: "Unlock 1-2 levels per week while learning chord theory to bridge intellectual understanding with instant auditory recognition.",
        zh: "在学习和弦三和弦、七和弦乐理的同时同步解锁对应关卡，实现乐理概念与听觉神经反射的瞬间握手。"
      },
      link: "https://completeeartrainer.com/"
    },
    {
      id: "tenuto-musictheory",
      name: "Tenuto & MusicTheory.net",
      category: "theory",
      level: "beginner",
      cost: "free",
      platforms: ["Web", "iOS"],
      tags: ["Clef Speed Drills", "Key Signatures", "Flashcards"],
      title: {
        en: "Tenuto (musictheory.net): High-Velocity Sight & Theory Flashcards",
        zh: "Tenuto (musictheory.net)：五线谱极速认音与调号速记利器"
      },
      summary: {
        en: "The definitive rapid-fire training app created by Ricci Adams' musictheory.net, offering configurable drills for note identification, intervals, and key signatures.",
        zh: "全球音乐学界广受赞誉的musictheory.net官方App，提供高度可配置的高频极速闪卡训练，涵盖高低音谱表识音与调号辨认。"
      },
      strengths: {
        en: "Completely customizable: limit drills to ledger lines, bass clef only, or specific accidentals; free on the web, cheap one-time app; zero ads.",
        zh: "参数完全自定义：可将训练范围精准锁定在上加线、下加线、纯低音谱表或特定调号；网页端终身免费，零广告干扰。"
      },
      caveats: {
        en: "Pure flashcard drills — exercises the eye-brain connection rather than hands-on-keys muscle memory.",
        zh: "纯粹的视觉-大脑概念闪卡；训练的是看谱认音的命名速度，仍需在键盘上通过真实触键把命名映射为手指空间坐标。"
      },
      synergy: {
        en: "If the bass clef slows you down, run 3 minutes of 50-note speed drills daily until note recognition becomes as automatic as reading the alphabet.",
        zh: "如果低音谱表让你卡壳，每天抽3分钟做50个音符的限时速认挑战，直到看见低音线上的音符如同阅读母语字母般不假思索。"
      },
      link: "https://www.musictheory.net/"
    },
    {
      id: "soundbrenner",
      name: "Soundbrenner / Metronome Apps",
      category: "theory",
      level: "all",
      cost: "free",
      platforms: ["iOS", "Android", "Wearables"],
      tags: ["Internal Pulse", "Subdivisions", "Tempo Ladder"],
      title: {
        en: "The Metronome as Truth Teller: Soundbrenner & Gap-Click Training",
        zh: "节拍器作为诚实面镜：Soundbrenner与空拍留白（Gap-Click）法则"
      },
      summary: {
        en: "The premier free, professional metronome app with precise visual feedback, custom polyrhythm subdivisions, and tempo increment trainers.",
        zh: "界面优雅出色的免费专业节拍器，提供高对比度视觉闪烁、复合节拍细分（Subdivision）与自动递增提速模式。"
      },
      strengths: {
        en: "Rock-solid timing engine; tap tempo; ability to silence clicks for 1-2 bars ('gap click') to brutally test your internal timekeeper.",
        zh: "毫秒级精准的稳定发音引擎；支持自定义设置“响两小节、静音两小节”的留白训练，残酷而真实地检验你的内心律动是否稳定。"
      },
      caveats: {
        en: "Many adults treat the metronome as an enemy or punishment; using it too fast too early breeds stiff hands and jaw clenching.",
        zh: "很多初学成人常把节拍器当成折磨自身的紧箍咒；在尚未梳理好肌肉轨迹前过早开节拍器狂赶，会导致全身肌肉僵死。"
      },
      synergy: {
        en: "Use it ONLY after a passage is completely mapped out error-free. Start at 50% target speed, increase by 2-3 BPM per successful iteration.",
        zh: "仅在某个难点小节已经能够毫无错音地慢速弹下后再开启。从目标速度的50%起步，每成功无误弹奏3遍，向上微调2-4 BPM。"
      },
      link: "https://www.soundbrenner.com/the-metronome/"
    },

    // --- 4. YouTube Mentors & Masterclasses ---
    {
      id: "josh-wright",
      name: "Josh Wright Piano (ProPractice)",
      category: "youtube",
      level: "intermediate",
      cost: "free",
      platforms: ["YouTube", "Web"],
      tags: ["Arm Weight", "Wrist Flexibility", "Slow Practice"],
      title: {
        en: "Dr. Josh Wright: The Master of Ergonomic Weight & Slow Practice Discipline",
        zh: "Josh Wright博士：手臂自然重量落地与微观慢练哲学的权威布道者"
      },
      summary: {
        en: "A Billboard #1 classical concert pianist and pedagogue whose YouTube channel offers gold-standard tutorials on wrist circularity, relaxation, and deliberate practice.",
        zh: "美国告示牌榜首古典钢琴家、密歇根大学音乐艺术博士，其频道是钻研手腕柔韧绕圈、消除弹奏紧张与微观慢练的宝藏级殿堂。"
      },
      strengths: {
        en: "Phenomenal focus on physical mechanics: explains *why* your forearm hurts and how dropping from the shoulder eliminates tension; specific deep-dives into classical literature.",
        zh: "极度注重生理发力力学：深刻剖析为何小臂酸痛、如何通过肩部下沉释放手臂重量彻底杜绝肌腱炎；单曲细拆详尽绝伦。"
      },
      caveats: {
        en: "High proportion of intermediate-to-advanced classical masterworks; can feel intimidating if you are still learning basic notes.",
        zh: "频道内容中高级古典文献较多；对于刚摸琴数周的零基础学员来说，部分技法讲解信息量过大，需按自身阶段检索其“Beginner Basics”。"
      },
      synergy: {
        en: "Watch his videos on 'wrist circles', 'dropping arm weight', and 'practice in rhythms' whenever your fingers feel stiff or a passage feels uneven.",
        zh: "只要你的手指感觉僵硬、跑动不均匀，必看其关于“手腕柔性绕环”（Wrist Circles）与“附点变节奏练习法”的经典专题视频。"
      },
      link: "https://www.youtube.com/@JoshWrightPiano"
    },
    {
      id: "nahre-sol",
      name: "Nahre Sol",
      category: "youtube",
      level: "all",
      cost: "free",
      platforms: ["YouTube"],
      tags: ["Creative Practice", "Harmonic Deconstruction", "Style Synthesis"],
      title: {
        en: "Nahre Sol: Artistic Deconstruction, Practice Journals & Stylistic Versatility",
        zh: "Nahre Sol（茱莉亚高材生）：练习笔记拆解、风格解构与艺术灵感"
      },
      summary: {
        en: "Juilliard graduate and composer whose visual practice journals demonstrate how a world-class musician deconstructs harmony, explores bebop/classical, and practices efficiently.",
        zh: "毕业于茱莉亚音乐学院的作曲家与钢琴家，用极富美感的视听语言呈现顶级音乐家如何解构和声结构、对比巴赫与比博普爵士，并记录真实练琴日记。"
      },
      strengths: {
        en: "Inspiring pedagogical mindset: shows that practice is playful exploration, not mindless repetition; deep visual breakdowns of chord voicings and textures.",
        zh: "重塑练琴心智：生动展示练琴不是枯燥重复，而是一场拆解和声拼图的趣味实验；对织体与和弦音位配置（Voicing）的剖析令人拍案叫绝。"
      },
      caveats: {
        en: "Not a step-by-step beginner syllabus; she creates conceptual deep-dives for curious minds rather than tutorial walkthroughs.",
        zh: "非保姆级手把手考级教程；更偏向于概念启发与审美开拓，适合作为拓展音乐格局与打破技术天花板的灵感源泉。"
      },
      synergy: {
        en: "Watch her 'How to Practice' journals to learn how to turn 4 chords of a simple tune into rich, layered variations at the keys.",
        zh: "观摩她的《练习的艺术》（Practice Journals），学习如何把一首简单作品的四个基本和弦拆解演化为层次丰满的自主即兴伴奏。"
      },
      link: "https://www.youtube.com/@NahreSol"
    },
    {
      id: "pianotv-allysia",
      name: "PianoTV (Allysia)",
      category: "youtube",
      level: "beginner",
      cost: "free",
      platforms: ["YouTube", "Web"],
      tags: ["Adult Repertoire", "RCM Grades", "Milestone Guidance"],
      title: {
        en: "PianoTV (Allysia): Warm, Honest, Realistic Repertoire Pathways for Adults",
        zh: "PianoTV（Allysia）：专为成人设计的诚恳、平实而系统的曲目进阶指南"
      },
      summary: {
        en: "A friendly Canadian piano instructor providing structured guides to piano repertoire grades (RCM 1-10), classical history, and compassionate advice for adult learners.",
        zh: "加拿大资深钢琴教师Allysia创办的亲切频道，按加拿大皇家音乐学院（RCM 1-10级）体系梳理最适合成年人弹奏的唯美古典曲目阶梯。"
      },
      strengths: {
        en: "Exceptional repertoire curation: tells you exactly which pieces are genuinely playable at Grade 1, 2, 3 so you don't jump into impossible pieces and despair.",
        zh: "极其精准的曲目难度把控：明确列出1到4级真正适合成人的好听乐曲，避免初学者盲目挑战《月光三》或肖邦叙事曲而导致的信心覆灭。"
      },
      caveats: {
        en: "Less focus on avant-garde or contemporary jazz techniques; focuses primarily on the traditional classical pathway.",
        zh: "偏向传统古典曲目与基础乐理梳理，较少涉及当代流行爵士即兴与前卫音乐。"
      },
      synergy: {
        en: "Consult her 'Easiest Pieces by Bach / Chopin / Beethoven' playlists when selecting your next self-directed repertoire project.",
        zh: "在规划自己下一个阶段的练习曲目时，先翻看她的“最易上手的巴赫/肖邦/萨蒂作品”精讲，按部就班登阶。"
      },
      link: "https://www.youtube.com/@PianoTV"
    },
    {
      id: "pianote",
      name: "Pianote (Lisa Witt)",
      category: "youtube",
      level: "beginner",
      cost: "free",
      platforms: ["YouTube", "Web"],
      tags: ["Pop Chords", "Singing Accompaniment", "Lead Sheets"],
      title: {
        en: "Pianote: Freedom from Sheet Music, Lead Sheets & Chords",
        zh: "Pianote（Lisa Witt）：流行和弦伴奏、即兴与告别死板看谱"
      },
      summary: {
        en: "A massive online community and YouTube channel focused on freeing adult learners from mechanical score-reading by teaching chords, rhythm grooves, and pop lead sheets.",
        zh: "风靡北美的流行即兴教学频道，致力于帮助成年人打破对传统五线谱的盲目恐惧，通过和弦记号（Lead Sheet）与节奏律动自由自弹自唱。"
      },
      strengths: {
        en: "Instant musicality: teaches left-hand patterns (broken fifths, arpeggios, syncopations) that allow you to play 100 pop songs within days; warm, joyful energy.",
        zh: "极强的实用伴奏能力：手把手传授左手黄金织体（分解八五度、流行琶音跑动、切分律动），短时间内即可为中外流行金曲配弹伴奏。"
      },
      caveats: {
        en: "Downplays traditional classical reading and contrapuntal voice-leading; will not teach you how to execute a Bach fugue.",
        zh: "对传统复调阅读与严谨古典对位训练较少涉及；如果你的目标是啃下巴赫或肖邦圆舞曲，此频道的知识体系需要古典向补充。"
      },
      synergy: {
        en: "Use Pianote lessons to learn how to accompany your family or yourself on songs like 'Chengdu' or popular folk tunes using chord charts.",
        zh: "当你想要在朋友聚会或家庭客厅为亲人伴奏唱歌、弹唱民谣或流行曲时，Pianote的和弦织体套路能直接产生立竿见影的成果。"
      },
      link: "https://www.youtube.com/@PianoteOfficial"
    },
    {
      id: "denis-zhdanov",
      name: "Denis Zhdanov / Taubman Ergonomics",
      category: "youtube",
      level: "advanced",
      cost: "free",
      platforms: ["YouTube"],
      tags: ["Taubman Approach", "Injury Prevention", "Physiological Freedom"],
      title: {
        en: "Denis Zhdanov: Physiological Mastery & Tension Elimination",
        zh: "Denis Zhdanov：陶布曼人体工学、力学生理学与零张力演奏"
      },
      summary: {
        en: "Concert pianist and profound pedagogue dedicated to biomechanics, physiological alignment, and the Taubman approach to piano playing without tension or pain.",
        zh: "乌克兰国际比赛大奖得主、资深钢琴家，深入阐释陶布曼（Taubman）演奏学派，致力于通过人体骨骼发力几何学彻底消除弹奏疼痛与生理紧张。"
      },
      strengths: {
        en: "Deepest anatomical explanations online: shows how finger isolation causes strain and how forearm rotation (pronation/supination) solves fast passages effortlessly.",
        zh: "全网最深度的解剖力学剖析：无情指出死练“手指独立高抬指”对肌腱的伤害，详解如何用“前臂轴向旋转”（Forearm Rotation）轻盈化解快速跑动。"
      },
      caveats: {
        en: "Highly analytical and technical; requires focused mental attention and body self-awareness to translate into tactile habit.",
        zh: "内容极其严谨抽象，属于大师级力学沉思；需要研习者具备细致的本体感觉（Proprioception）才能将视频内化为肌体微动作。"
      },
      synergy: {
        en: "Mandatory viewing if you experience forearm burning, thumb tension, or wrist soreness after 30 minutes of practice.",
        zh: "如果无论练车尔尼还是哈农，只要超过半小时小臂外侧就会酸胀或大拇指根部发硬，必须立即观看其旋转力学系列讲座进行自查纠错。"
      },
      link: "https://www.youtube.com/@DenisZhdanov"
    },
    {
      id: "michael-maiber",
      name: "Michael Maiber",
      category: "youtube",
      level: "all",
      cost: "free",
      platforms: ["YouTube"],
      tags: ["Ludovico Einaudi", "Step-by-Step", "Self-Taught Path", "Contemporary"],
      title: {
        en: "Michael Maiber: The Patient, Phrase-by-Phrase Mentor for Ludovico Einaudi & Contemporary Piano",
        zh: "Michael Maiber：艾诺迪（Ludovico Einaudi）与现代新古典钢琴曲目的逐句慢拆导师"
      },
      summary: {
        en: "A beloved self-taught pianist whose channel is an indispensable treasure for learning Ludovico Einaudi (Nuvole Bianche, Divenire, Una Mattina, Experience), Yann Tiersen, and Chopin through clear, patient phrase-by-phrase visual breakdowns.",
        zh: "一位深受钢琴自学者喜爱的自学成才钢琴博主。其频道是攻克鲁多维科·艾诺迪（《Nuvole Bianche》《Divenire》《Una Mattina》《Experience》等）、扬·提尔森与肖邦名作的宝藏，以极度耐心、逐句慢速拆解著称。"
      },
      strengths: {
        en: "Perfect bridge for the self-taught struggling adult: demonstrates both hands separately, highlights exact fingerings, and breaks intimidating continuous rolling arpeggios and cross-rhythms into digestible, playable phrases.",
        zh: "极其贴合自学者‘痛苦卡点’的拆解节奏：不仅有双手慢速特写，还细致拆解难啃的长琶音跑动与左右手复合节奏，将宏大篇幅切分成极其亲民的2小节练习单元。"
      },
      caveats: {
        en: "Focuses on practical performance execution and musical phrasing rather than abstract academic music theory or formal classical analysis.",
        zh: "重在单曲演奏实践与音乐情感句法，非学院派枯燥理论讲座，适合作为攻克具体心仪曲目的实战导师。"
      },
      synergy: {
        en: "The author learned nearly all of his Ludovico Einaudi repertoire from this channel through persistent struggle. Pair Michael's tutorials with the Urtext/official score to master hand independence and rhythmic flow.",
        zh: "本指南作者的大部分艾诺迪曲目，皆是对照着 Michael Maiber 的教学视频并结合自身不断抗争摸索而攻克的。建议手边备好官方乐谱，配合他的慢速示范反复打磨左右手独立性。"
      },
      link: "https://www.youtube.com/@MichaelMaiber"
    },
    {
      id: "katherine-cordova",
      name: "Katherine Cordova",
      category: "youtube",
      level: "all",
      cost: "free",
      platforms: ["YouTube", "Musicnotes", "Spotify"],
      tags: ["Rock & Film Covers", "Self-Taught", "Expressive Touch", "Cinematic Solo"],
      title: {
        en: "Katherine Cordova: Self-Taught Artistry, Cinematic Grand Arrangements & Raw Emotional Depth",
        zh: "Katherine Cordova：自学成才的艺术典范、电影感独奏改编与直击心灵的音色层次"
      },
      summary: {
        en: "A world-class self-taught Canadian pianist featured on Billboard and ClassicFM, renowned for emotionally devastating, dynamically explosive acoustic piano covers of rock, film, and alternative epics.",
        zh: "曾登顶 Billboard、受 ClassicFM 赞誉的加拿大自学成才钢琴家，以极具情感张力、气势磅礴且细腻动人的纯钢琴独奏改编（摇滚、影视配乐与经典名曲）风靡全网。"
      },
      strengths: {
        en: "Supreme master of pianistic dynamics and voice-leading without academic dogmatism: demonstrates how deep listening, left-hand bass independence, and singing touch transform pop and rock into serious, evocative solo literature.",
        zh: "极致的动态掌控力与声部歌唱性：生动展示了如何在摒弃学院派死板教条的同时，通过深邃的内听、极其果敢的强弱对比与左手低音独立性，将流行摇滚转化为殿堂级的现代独奏艺术。"
      },
      caveats: {
        en: "Her solo arrangements are technically demanding and polyphonically dense (graded at intermediate to advanced concert level); not simple one-finger beginner tutorials.",
        zh: "其改编曲目充满丰富的低音跨度、内声部跑动与复调交织，大多处于中高级演奏水准，非简单的单音旋律入门教程。"
      },
      synergy: {
        en: "Watch her performances as the ultimate aesthetic antidote to mechanical practice fatigue. Her work is a featured cornerstone in our 'Resources I Enjoy' section.",
        zh: "在被枯燥的练习曲或机械打卡消耗殆尽时，聆听她的演奏是重燃练琴爱意的最佳审美解药；本指南在‘挚爱资源与灵感’专栏中重点推介。"
      },
      link: "https://www.youtube.com/@KatherineCordova"
    },

    // --- 5. Method Books, Etudes & Literature ---
    {
      id: "alfred-adult",
      name: "Alfred's Basic Adult All-in-One Course",
      category: "books",
      level: "beginner",
      cost: "paid",
      platforms: ["Print", "Kindle"],
      tags: ["Harmonic Approach", "Chords Early", "Pedagogical Standard"],
      title: {
        en: "Alfred's Basic Adult All-in-One Course (Books 1, 2, 3)",
        zh: "阿尔弗莱德《成人全音阶钢琴基础教程》（Alfred's Basic Adult All-in-One）"
      },
      summary: {
        en: "The best-selling adult piano curriculum worldwide, integrating theory, technique, and repertoire into a continuous harmonic progression.",
        zh: "全球销量第一的成年人钢琴自学启蒙教材，将乐理知识、基本指法与阶段性曲目融为一体，采用成熟的和弦导向教学法。"
      },
      strengths: {
        en: "Introduces chords (I, IV, V7) in week two, allowing adults to make rich harmonic sound immediately; progressive introduction of ledger lines and syncopation.",
        zh: "第二周即引入左手三和弦与属七和弦，让成人从一开始就听到丰满和谐的声音；理论与小测验穿插合理，循序渐进。"
      },
      caveats: {
        en: "Early repertoire contains many old American folk tunes and public domain marches that can feel musically cheesy to some learners; leaves the hands in fixed C/G positions for slightly too long.",
        zh: "前期收录了较多古老美国民谣和简单改编曲，艺术审美稍显陈旧；左右手固定在C五指音位和G音位的时间较长，需警惕手指僵化定位。"
      },
      synergy: {
        en: "Work through Book 1 for 4-6 months to establish reliable chord and notation literacy, supplementing with real pieces like Satie or Bach.",
        zh: "用4到6个月踏实走完第一册，筑牢读谱与和弦基石；期间同步搭配萨蒂《裸体歌舞》或巴赫小步舞曲等真正的高雅作品调剂。"
      },
      link: "https://www.alfred.com/alfreds-basic-adult-piano-course/"
    },
    {
      id: "faber-adult",
      name: "Faber Adult Piano Adventures",
      category: "books",
      level: "beginner",
      cost: "paid",
      platforms: ["Print", "App"],
      tags: ["Musicality", "Hand Movement", "Companion App"],
      title: {
        en: "Faber Adult Piano Adventures (Books 1 & 2)",
        zh: "菲伯尔《成人钢琴探险之旅》（Faber Adult Piano Adventures）"
      },
      summary: {
        en: "The chief modern rival to Alfred, created by Nancy and Randall Faber, renowned for more artistic arrangements, earlier keyboard mobility, and a companion app.",
        zh: "现代钢琴教育泰斗菲伯尔夫妇专为成人编著的权威教程，以更具现代美感的编曲、更早鼓励全键盘移动以及配套高品质音频闻名。"
      },
      strengths: {
        en: "Encourages keyboard movement across octaves early rather than locking into 5-finger prisons; arrangements sound genuinely satisfying; outstanding visual typography.",
        zh: "打破五指固定手位的牢笼，从前几课就引导手指在八度音区间自由呼吸移动；示范音频极为考究，伴奏丰富好听。"
      },
      caveats: {
        en: "Slightly less emphasis on dense chord analysis compared to Alfred; advances quickly into two-hand independence in Book 2.",
        zh: "在纯和声符号分析上的篇幅略逊于阿尔弗莱德；进入第二册后双手独立性跨度较大，需放慢练习周期。"
      },
      synergy: {
        en: "Preferred over Alfred for learners with an aesthetic preference for classical and film lyricism over traditional American folk songs.",
        zh: "如果你的审美更偏向古典的优美线条或现代电影配乐，菲伯尔的曲目编写体验通常明显优于老派教程。"
      },
      link: "https://pianoadventures.com/adult/"
    },
    {
      id: "mikrokosmos-bartok",
      name: "Béla Bartók: Mikrokosmos (Vol. 1-3)",
      category: "books",
      level: "beginner",
      cost: "free",
      platforms: ["Print", "IMSLP"],
      tags: ["Polyphony", "Hand Independence", "Modal Ears"],
      title: {
        en: "Béla Bartók: Mikrokosmos — The Secret Weapon for Two-Hand Independence",
        zh: "巴托克《小宇宙》（Mikrokosmos 第一至三卷）：双手独立与调式听觉的终极密器"
      },
      summary: {
        en: "153 progressive pedagogical miniatures composed by Hungarian master Béla Bartók to teach genuine polyphony, asymmetrical rhythms, and absolute two-hand autonomy.",
        zh: "匈牙利二十世纪伟大作曲家巴托克写给世人的153首渐进微型宝藏，专为训练双手绝对独立、复调思维与奇妙民间调式而生。"
      },
      strengths: {
        en: "Treats the left hand as an equal singing voice from piece #1, completely curing 'lazy left hand syndrome'; trains rhythmic independence unlike any standard pop/folk method.",
        zh: "从第1课开始就把左手视作与右手地位完全平等的歌唱旋律，彻底治愈成人初学者“左手只会伴奏傻按”的顽疾；节奏思维超越常规4/4拍教条。"
      },
      caveats: {
        en: "Non-traditional harmonies (modal, folk, eastern European) can sound strange to ears accustomed solely to modern pop major/minor chords.",
        zh: "采用东欧调式与不协和音阶，旋律风格清新古怪；习惯了流行大调套路耳朵的人初听可能会感到陌生，但一旦适应将极大拓宽听觉带宽。"
      },
      synergy: {
        en: "Play 1 miniature per day as a 5-minute brain coordination warm-up before working on your main pieces.",
        zh: "每天在练琴前花5分钟攻克一首半页长的小宇宙微型曲目，将其作为双手分脑协调的“每日智力体操”。"
      },
      link: "https://imslp.org/wiki/Mikrokosmos,_Sz.107_(Bart%C3%B3k,_B%C3%A9la)"
    },
    {
      id: "burgmuller-op100",
      name: "Burgmüller: 25 Progressive Etudes Op. 100",
      category: "books",
      level: "intermediate",
      cost: "free",
      platforms: ["Print", "IMSLP"],
      tags: ["Lyrical Etudes", "Arabesque", "Phrasing & Drama"],
      title: {
        en: "Burgmüller Op. 100: 25 Romantic Etudes That Sound Like Real Music",
        zh: "布格缪勒《简易与进阶25首练习曲 Op. 100》：饱含浪漫诗意的进阶试金石"
      },
      summary: {
        en: "The universally beloved transitional collection between beginner methods and intermediate masterworks, containing famous pieces like *Arabesque*, *Ballade*, and *Pastorale*.",
        zh: "全球钢琴界从初级向中级跃升的必备浪漫主义宝库，收录了家喻户晓的《阿拉伯风》（Arabesque）、《叙事曲》（Ballade）与《牧歌》。"
      },
      strengths: {
        en: "Every etude is a complete, emotionally thrilling musical story rather than dry mechanical drills (unlike Czerny); teaches phrasing, balance, and pedaling organically.",
        zh: "每一首都是一幅跌宕起伏的动人微型画卷，毫无车尔尼式的枯燥乏味；在极具戏剧性的旋律中浑然天成地教会你连跳音、歌唱性与踏板艺术。"
      },
      caveats: {
        en: "Fast tempo markings can seduce beginners into rushing; must be learned at half tempo with careful arm weight before attempting performance speed.",
        zh: "标定速度颇快，容易诱导成人初学者为了追求爽感而虚浮狂飙；必须在极慢速度下用沉稳的手臂重量做足微细音色雕琢。"
      },
      synergy: {
        en: "A must-tackle milestone before attempting demanding works like Handel's *Passacaglia* or Chopin Preludes.",
        zh: "在向亨德尔《帕萨卡利亚》或肖邦前奏曲发起冲锋前的关键必修站，用《阿拉伯风》检验手指灵敏度，用《叙事曲》检验左手戏剧和弦。"
      },
      link: "https://imslp.org/wiki/25_%C3%89tudes_faciles_et_progressives,_Op.100_(Burgm%C3%BCller,_Friedrich)"
    },
    {
      id: "chang-fundamentals",
      name: "Fundamentals of Piano Practice (Chuan C. Chang)",
      category: "books",
      level: "all",
      cost: "free",
      platforms: ["Web", "PDF"],
      tags: ["Mental Play", "Post-Practice Improvement", "Micro-Chunking"],
      title: {
        en: "Fundamentals of Piano Practice: Scientific Practice Method & PPI",
        zh: "张全成《钢琴弹奏的艺术与科学》（Fundamentals of Piano Practice）"
      },
      summary: {
        en: "The world's most famous open-source manual on the science of piano practice, exploring Hands Separate (HS) routines, micro-segmentation, and Post-Practice Improvement (PPI).",
        zh: "互联网上传播最广的开源钢琴练习科学文献，从物理学与脑认知科学视角，彻底解析“分手法（HS）”、“微观切片”与“练后睡眠神经巩固效应（PPI）”。"
      },
      strengths: {
        en: "Explodes conventional myths of endless mindless repetition; explains how the brain synthesizes motor skills overnight during deep sleep; teaches segment boundary stitching.",
        zh: "彻底粉碎“机械苦练万遍自成”的陈腐迷信；深刻揭示大脑在夜间深度睡眠中重构髓鞘神经通路的核心规律，传授小节接口缝合术。"
      },
      caveats: {
        en: "The tone can be dogmatic and opinionated in some chapters; some claims on memory have been debated, but the deliberate practice principles are pure gold.",
        zh: "文风带有理工学者的绝对与执拗，部分关于记忆机制的叙述略有主观色彩，但其总结的双手拆解与睡眠沉淀原则绝对是自学无价之宝。"
      },
      synergy: {
        en: "Read the first two chapters before learning any complex piece like *Forrest Gump* or *Passacaglia* to cut your practice hours in half.",
        zh: "在攻克任何有技术难度的乐曲（如《阿甘正传》连续琶音或《帕萨卡利亚》交替跨度）前，先阅读其前两章，能为你省去一半无意义的机械弯路。"
      },
      link: "https://fundamentals-of-piano-practice.readthedocs.io/"
    },
    {
      id: "effortless-mastery",
      name: "Effortless Mastery (Kenny Werner)",
      category: "books",
      level: "all",
      cost: "paid",
      platforms: ["Print", "Audiobook"],
      tags: ["Mindset", "Releasing Fear", "Somatic Ease"],
      title: {
        en: "Effortless Mastery: Liberating the Master Musician Within",
        zh: "肯尼·维尔纳《自如的驾驭》（Effortless Mastery）：释放深层音乐本能与消解焦虑"
      },
      summary: {
        en: "A transformational psychological text by jazz master Kenny Werner addressing the acute anxiety, self-criticism, and physical paralysis that plagues adult learners.",
        zh: "爵士泰斗肯尼·维尔纳撰写的身心觉察圣经，专治成年人在琴键前的严苛内耗、自我审查、恐惧感以及因心理焦虑引发的手部生理僵直。"
      },
      strengths: {
        en: "Teaches the core mantra: 'Step 1 is effortless space; if a single note feels tense, stop immediately.' Cures the adult habit of self-punishing practice.",
        zh: "建立至简信条：“第一步永远是身体的极度松弛；只要按下一个琴键让你感到紧张或压迫，立刻停下。”彻底根治自残式逼迫练习。"
      },
      caveats: {
        en: "Philosophical and meditative in orientation rather than a tactical index of finger exercises; requires openness to somatic self-awareness.",
        zh: "偏向心理学与沉思冥想体验，而非具象的指法练习谱集；需要研习者愿意对自己的呼吸、肩颈与情绪保持诚实的内观。"
      },
      synergy: {
        en: "Whenever you sit down at the piano feeling stressed from work or discouraged by slow progress, read one page to center your mind.",
        zh: "每当结束一整天繁重工作坐在琴凳前感到心浮气躁，或是因某个小节屡屡弹错而萌生沮丧退意时，读上几页，琴音会重归清澈澄明。"
      },
      link: "https://kennywerner.com/effortless-mastery"
    },

    // --- 6. Hardware, Acoustics & Ergonomics ---
    {
      id: "weighted-digital-piano",
      name: "88-Key Weighted Hammer Action Digital Pianos",
      category: "hardware",
      level: "all",
      cost: "paid",
      platforms: ["Hardware"],
      tags: ["Roland FP-30X", "Yamaha P-225", "Kawai ES120", "PHA-4"],
      title: {
        en: "The Essential Foundation: 88 Weighted Keys with Authentic Escapement",
        zh: "自学基石设备：88键重锤击弦感电钢琴与擒纵触感选型"
      },
      summary: {
        en: "Why unweighted 61-key plastic keyboards cripple adult motor memory, and what to look for in benchmark models (Roland FP-10/30X, Yamaha P-145/225, Kawai ES120).",
        zh: "为什么61键弹簧塑料电子琴会从第一天起摧毁成人的触键深度，以及如何选购三大标杆型号（Roland PHA-4、Yamaha GHS/GTS、Kawai RHC）。"
      },
      strengths: {
        en: "Headphone jack enables unlimited late-night practice without disturbing kids or neighbors; zero tuning costs; pristine USB-MIDI connection to apps and DAWs.",
        zh: "耳机接口实现深夜无限畅弹，完全不扰家人与邻里；终身无需昂贵调律；USB-MIDI一线直连各类数字软件与制谱工具。"
      },
      caveats: {
        en: "Entry-level keybeds lack the infinite acoustic sympathetic resonance of an acoustic soundboard; cheaper spring actions do not build real finger extensor strength.",
        zh: "入门电钢即使带重锤，仍缺乏真钢音板在琴腔内的空气物理共鸣；切忌购买几百元无重锤的弹簧琴，那将导致你在真琴上完全按不下琴键。"
      },
      synergy: {
        en: "Equip with a sturdy wooden or heavy double-X stand (no wobbling) and an adjustable bench so your elbows rest exactly level with the white keys.",
        zh: "务必搭配稳固沉重的木质底架（严禁摇晃）与可升降琴凳，确保小臂在自然平放触键时与白键表面保持水平或微高于键面。"
      },
      link: "https://www.roland.com/us/products/fp_series/"
    },
    {
      id: "continuous-sustain-pedal",
      name: "Half-Damper Continuous Sustain Pedals",
      category: "hardware",
      level: "intermediate",
      cost: "paid",
      platforms: ["Hardware"],
      tags: ["Roland DP-10", "Yamaha FC3A", "Half-Pedaling", "Acoustic Realism"],
      title: {
        en: "The Lungs of the Piano: Continuous Half-Damper Sustain Pedals",
        zh: "钢琴的灵魂之肺：支持半踏板（Half-Damper）的金属连续延音踏板"
      },
      summary: {
        en: "Upgrading from cheap plastic on/off footswitches to a continuous potentiometer pedal (like Roland DP-10 or Yamaha FC3A) that simulates real damper felt lifting.",
        zh: "从电钢包装里附带的廉价塑料方形开关踏板，升级为支持连续阻尼深度采样的专业金属延音踏板（如Roland DP-10、Yamaha FC3A）。"
      },
      strengths: {
        en: "Allows 'half-pedaling': softening the bass wash without cutting the singing treble, preventing the muddy cacophony that ruins adult expressive playing.",
        zh: "解锁“半踏板”（Half-Pedaling）与羽化换音技术：在换和弦时让低音浑浊声快速散去，同时保留高音余音袅袅，告别踩死踏板的一锅粥浑浊感。"
      },
      caveats: {
        en: "Your digital piano must support continuous half-pedal input via its sustain jack (check specifications).",
        zh: "需确认你的电钢琴背板Sustain接口本身支持连续电位器信号（市面主流如Roland FP-10/30X、Yamaha均支持）。"
      },
      synergy: {
        en: "Crucial for romantic and soundtrack pieces like *Passacaglia*, *Forrest Gump*, and Satie, where pedal articulation dictates emotional clarity.",
        zh: "攻克如《帕萨卡利亚》《阿甘正传》《成都》这类抒情大作时不可或缺，踏板的细腻呼吸决定了音乐究竟是高级清澈还是嘈杂混乱。"
      },
      link: "https://www.roland.com/us/products/dp-10/"
    },
    {
      id: "open-back-headphones",
      name: "Open-Back Audiophile Studio Headphones",
      category: "hardware",
      level: "all",
      cost: "paid",
      platforms: ["Hardware"],
      tags: ["Sennheiser HD560S", "Audio-Technica", "Fatigue-Free Listening"],
      title: {
        en: "Fatigue-Free Soundstage: Open-Back Reference Headphones",
        zh: "无听觉疲劳的声场：开放式发烧级监听耳机（Sennheiser HD560S等）"
      },
      summary: {
        en: "Why closed gaming headsets cause ear fatigue within 20 minutes, and why open-back acoustic headphones recreate the natural air-resonance of a grand piano.",
        zh: "解析为何封闭式降噪/游戏耳机会在20分钟内导致鼓膜气压疲劳，以及开放式声学设计如何还原三角钢琴在空气中扩散的真实三维声场。"
      },
      strengths: {
        en: "Zero ear pressure build-up; natural spatial localization makes it feel like the sound is coming from the instrument in front of you rather than trapped in your skull.",
        zh: "空气完全流通，零气压堆积；声场极其开阔自然，让你感觉声音是从眼前的琴体空间共振发出，而非被硬塞进颅脑内。"
      },
      caveats: {
        en: "Sound leaks slightly into the room (anyone sitting right next to you will hear faint tinny clicks).",
        zh: "开放式外罩会有微量声音外泄（同处一室近距离的人能听到轻微金属按键声），但不影响隔壁房间睡眠。"
      },
      synergy: {
        en: "Pair Sennheiser HD560S or Philips SHP9500 with a 3-meter cord so you can move your upper torso freely without cord tugging.",
        zh: "选用配备3米长线缆的森海塞尔HD560S或飞利浦SHP9500，确保身体躯干与双臂随音乐起伏摆动时耳机线毫无牵扯感。"
      },
      link: "https://www.sennheiser-hearing.com/en-US/p/hd-560s/"
    },
    {
      "id": "skoove",
      "name": "Skoove",
      "category": "apps",
      "level": "beginner",
      "cost": "paid",
      "platforms": [
            "iOS",
            "Android",
            "Web"
      ],
      "tags": [
            "Interactive Theory",
            "Audio Listening",
            "Improvisation Starter"
      ],
      "title": {
            "en": "Skoove: Holistic Theory-Integrated Interactive Method",
            "zh": "Skoove：乐理融合、听音互动与即兴起步交互教程"
      },
      "summary": {
            "en": "A browser and mobile learning app blending step-by-step popular and classical songs with live acoustic pitch recognition and foundational theory concepts.",
            "zh": "兼顾网页端与移动端的交互式学琴平台，将流行金曲与古典小品按和声概念拆解，结合麦克风即时拾音评测。"
      },
      "strengths": {
            "en": "Stronger musical theory integration than Simply Piano; teaches question-and-answer phrasing and basic improvisation early.",
            "zh": "相比纯游戏化App更注重乐理渗透；很早便引入“问答句式”（Question-and-Answer）与初级旋律即兴思维。"
      },
      "caveats": {
            "en": "Web audio detection can lag on low-power microphones; repertoire library is smaller than Flowkey or Piano Marvel.",
            "zh": "网页端依赖浏览器音频拾音，在设备性能受限时偶有识别延迟；整体曲库体量略小于Flowkey与Piano Marvel。"
      },
      "synergy": {
            "en": "Great for adult beginners who want bite-sized theory explanations embedded directly into short interactive song tutorials.",
            "zh": "适合希望在练习短小好听旋律的同时，顺便弄懂基本和弦构成与调式逻辑的零基础成年人。"
      },
      "link": "https://www.skoove.com/"
},
    {
      "id": "sheetmusicdirect",
      "name": "Sheet Music Direct & Musicnotes",
      "category": "scores",
      "level": "all",
      "cost": "paid",
      "platforms": [
            "Web",
            "iOS",
            "Android"
      ],
      "tags": [
            "Official Arrangements",
            "Soundtracks",
            "Instant Transposition"
      ],
      "title": {
            "en": "Sheet Music Direct: Authorized Contemporary & Soundtrack Scores",
            "zh": "Sheet Music Direct / Musicnotes：正版授权当代影视与流行金曲乐谱库"
      },
      "summary": {
            "en": "The premier licensed repositories for authentic digital sheet music, providing Hal Leonard and major studio authorized arrangements of film scores, anime, and pop.",
            "zh": "全球最大的正版数字乐谱商城，提供由Hal Leonard等顶级音乐出版社官方授权的影视原声、动漫配乐与流行金曲高质量编曲。"
      },
      "strengths": {
            "en": "High typographical accuracy; arrangements written by professional arrangers with reliable fingerings; instant digital transposition before printing.",
            "zh": "印刷精度极高、错音率为零；由专业编曲家编排且附带可信的指法标记；支持在网页端直接一键试听并任意升降调后再打印。"
      },
      "caveats": {
            "en": "Paid per score ($3.99 - $5.99 per title) or via PASS subscription; not suitable if you are exclusively studying public domain baroque/classical works.",
            "zh": "单首乐谱按次付费（每首约3-6美元）或按年订阅；若主要钻研巴洛克与经典古典文献，IMSLP公版库更具性价比。"
      },
      "synergy": {
            "en": "Whenever you want to play a clean, authoritative solo piano arrangement of *Forrest Gump*, *Game of Thrones*, or Joe Hisaishi, buy the official arrangement here.",
            "zh": "当你准备精研《阿甘正传》《权力的游戏》或久石让的纯钢琴独奏版时，直接购买官方认证谱，省去社区谱错漏百出的试错损耗。"
      },
      "link": "https://www.sheetmusicdirect.com/"
},
    {
      "id": "moises-ai",
      "name": "Moises.ai & Stem Separation",
      "category": "theory",
      "level": "intermediate",
      "cost": "free",
      "platforms": [
            "Web",
            "iOS",
            "Android"
      ],
      "tags": [
            "Stem Separation",
            "Audio Slow-Mo",
            "Chord Detection"
      ],
      "title": {
            "en": "Moises.ai: AI Audio Stem Isolation & Slow-Motion Practice",
            "zh": "Moises.ai：AI音轨无损剥离、和弦拾取与慢放伴练"
      },
      "summary": {
            "en": "An AI audio separation platform that strips songs into isolated piano, vocal, and drum stems, allowing seamless pitch and tempo changes without distortion.",
            "zh": "领先的人工智能音频分离工具，能将任何歌曲瞬间剥离出独立的纯钢琴、纯人声与伴奏声部，支持无损变速与和弦检测。"
      },
      "strengths": {
            "en": "Mute the original piano in a song to play the live solo yourself; slow dense master recordings down to 60% to transcribe subtle voicings.",
            "zh": "可将原曲中的钢琴轨静音，自己跟着原声交响乐团伴奏实弹；将大师飞速录音慢放至60%，清晰扒出左手幽微的和声织体。"
      },
      "caveats": {
            "en": "Free tier has track limits per month; complex symphonic recordings can exhibit minor acoustic phase artifacts.",
            "zh": "免费账户每月有上传数量限制；在极其繁复的管弦乐大齐奏中偶尔存在微量相位杂音。"
      },
      "synergy": {
            "en": "Use when learning lyrical pieces like *Chengdu* or contemporary ballads to absorb the exact timing and rubato of the original singer or pianist.",
            "zh": "在研习《成都》等抒情民谣时，单开纯吉他/贝斯伴奏，让自己的右手在琴键上代替歌手原声歌唱，极具演奏沉浸感。"
      },
      "link": "https://moises.ai/"
},
    {
      "id": "open-studio",
      "name": "Open Studio (Peter Martin & Adam Maness)",
      "category": "youtube",
      "level": "intermediate",
      "cost": "free",
      "platforms": [
            "YouTube",
            "Web"
      ],
      "tags": [
            "Jazz Harmony",
            "Voicings",
            "Rhythmic Swing"
      ],
      "title": {
            "en": "Open Studio: Jazz Piano Voicings, Soulful Grooves & Chord Chemistry",
            "zh": "Open Studio（Peter Martin）：爵士和声音位配置、律动与即兴精髓"
      },
      "summary": {
            "en": "Grammy-winning jazz pianist Peter Martin and Adam Maness provide the internet's most infectious and groove-centered piano masterclasses on voicings and ear playing.",
            "zh": "格莱美奖得主Peter Martin携手资深音乐人创办的顶级频道，深入浅出传授爵士和声配置（Rootless Voicings）、摇摆律动与耳朵先行的即兴之道。"
      },
      "strengths": {
            "en": "Incredible charisma and ear-centered teaching; breaks down rootless voicings (A-form, B-form) so any pop ballad sounds like rich, smoky velvet; heavy emphasis on pulse.",
            "zh": "极其生动的耳朵驱动教学法；深度传授“无根音和弦音位配置”（Rootless Voicings），让简单的流行曲瞬间披上如丝绒般深邃奢华的爵士质感。"
      },
      "caveats": {
            "en": "Focused on jazz, blues, and contemporary chords rather than classical conservatory literature.",
            "zh": "重心集中于爵士和声、蓝调律动与当代和弦即兴，对传统古典严谨记谱复调涉及较少。"
      },
      "synergy": {
            "en": "Whenever your left hand feels boring playing simple root-position fifths, watch their 2-5-1 voicing videos to transform your accompaniment harmony.",
            "zh": "只要你厌倦了左手只会弹单调的五度根音伴奏，必看其2-5-1黄金和弦配置课，一举跃迁伴奏和声审美。"
      },
      "link": "https://www.openstudiojazz.com/"
},
    {
      "id": "bach-inventions",
      "name": "J.S. Bach: Two-Part Inventions (BWV 772–786)",
      "category": "books",
      "level": "intermediate",
      "cost": "free",
      "platforms": [
            "Print",
            "IMSLP"
      ],
      "tags": [
            "Polyphony",
            "Two-Voice Counterpoint",
            "Architectural Mastery"
      ],
      "title": {
            "en": "J.S. Bach: Two-Part Inventions — The Summit of Bilateral Hand Brain Autonomy",
            "zh": "J.S.巴赫《二部创意曲》（BWV 772-786）：左右脑完全分化与复调巅峰"
      },
      "summary": {
            "en": "15 contrapuntal masterpieces composed by J.S. Bach specifically as a textbook to teach keyboardists how to make two independent voices converse with clarity and cantabile touch.",
            "zh": "音乐之父巴赫亲自为键盘研习者谱写的15首对位法教科书，旨在训练演奏者如何让两只手各自化身为独立的歌唱家，在键盘上展开激辩与对话。"
      },
      "strengths": {
            "en": "Unrivaled cognitive and motor rewiring; once you can play Invention No. 1 or No. 8, your hands will never again feel confused by counter-rhythms or independent dynamics.",
            "zh": "无与伦比的大脑运动神经重构价值；一旦你亲手攻克第1首（C大调）或第8首（F大调），你的双手将彻底告别节拍打架，左右手独立性将终身受益。"
      },
      "caveats": {
            "en": "Intense mental friction: requires learning each voice completely by heart before merging hands; cannot be skimmed or played mindlessly.",
            "zh": "极高的心智算力摩擦：在双手合体前，必须做到单手能一边弹一边唱出另一声部的旋律；任何不走心的机械盲刷都会在此惨败。"
      },
      "synergy": {
            "en": "The essential prerequisite study for tackling Handel-Halvorsen's *Passacaglia* or complex Chopin polyphonic textures.",
            "zh": "攻克亨德尔-哈尔沃森《帕萨卡利亚》等交响级复调变奏前的必由之路，每一小节都是纯净的音乐逻辑钻石。"
      },
      "link": "https://imslp.org/wiki/15_Inventions,_BWV_772-786_(Bach,_Johann_Sebastian)"
},
    {
      "id": "ergonomic-bench",
      "name": "Adjustable Hydraulic Piano Bench & Ergonomic Posture",
      "category": "hardware",
      "level": "all",
      "cost": "paid",
      "platforms": [
            "Hardware"
      ],
      "tags": [
            "Adjustable Height",
            "Elbow Level",
            "Spine Health"
      ],
      "title": {
            "en": "The Unsung Physical Anchor: Adjustable Bench & Biomechanical Alignment",
            "zh": "被遗忘的生理基石：可升降液压琴凳与人体工学坐姿对齐"
      },
      "summary": {
            "en": "Why sitting at the wrong height causes chronic shoulder pain and finger collapse, and how to establish the golden triangle of keyboard seating.",
            "zh": "解析为何坐高偏差2厘米会导致慢性颈肩酸痛与手指塌陷，以及如何构建键盘演奏黄金三角坐姿。"
      },
      "strengths": {
            "en": "Allows your elbows to sit parallel or slightly above white key height; provides a stable base on your sit-bones (ischial tuberosities) so your upper body can lean with zero lumbar strain.",
            "zh": "确保手肘自然平放时与琴键保持水平或微高2厘米；让体重稳稳落在坐骨结节前侧，躯干随音符自由倾斜而腰椎零负担。"
      },
      "caveats": {
            "en": "Quality hydraulic benches can be heavy and cost $120–$250; but standard dining chairs are almost always too low or too soft, crippling arm weight.",
            "zh": "高品质液压升降凳自重较沉且有一定购入成本；但普通的餐椅或电脑软椅高度几乎普遍偏低且无支撑，会直接锁死手臂重力的释放。"
      },
      "synergy": {
            "en": "Sit on the front half of the bench, feet flat on the floor (or right foot gently hovering over the sustain pedal). Feel rooted to the earth.",
            "zh": "坐满琴凳的前三分之一，双脚稳稳踩实地面（右脚轻浮于延音踏板上方），全身形成稳固的人体大理石基座。"
      },
      "link": "https://www.andexinger.de/en/products/piano-benches/"
}
  ],

  // Side-by-side Comparative Analysis (5 Pairs)
  comparators: [
    {
      id: "synthesia-vs-sheet",
      topic: {
        en: "Waterfall / Falling Notes (Synthesia) vs. Traditional Sheet Music",
        zh: "下落瀑布流（Synthesia） 对决 传统五线谱识读"
      },
      overview: {
        en: "The debate between instantaneous visual gratification and long-term musical autonomy.",
        zh: "在“即时视觉多巴胺反馈”与“长期音乐自主权”之间的深刻权衡。"
      },
      left: {
        title: { en: "Waterfall Visualizer (Synthesia)", zh: "下落瀑布流 / 视频光条模式" },
        pros: {
          en: ["Day 1 gratification: play a 4-bar tune in 15 minutes", "Instant spatial view of hand jumps and key locations", "No music theory barrier for absolute beginners"],
          zh: ["首日即时满足感：15分钟内即可照猫画虎敲出旋律", "极其直观地看清双手跨度与键盘物理空间分布", "零乐理门槛，无需经历识谱痛苦期"]
        },
        cons: {
          en: ["Extreme cognitive drag: memorizing thousands of color bars is unsustainable", "Zero rhythmic comprehension; reliant on external visual tempo", "Leaves the player musically illiterate the moment the screen turns off"],
          zh: ["长线认知负荷巨大：死记硬背成千上万个色块极度耗神且易忘", "完全无法内化节拍与声部结构，沦为屏幕的提线木偶", "一旦离开屏幕瞬间失忆，丧失所有自学新曲的独立能力"]
        }
      },
      right: {
        title: { en: "Traditional Sheet Music (Notation)", zh: "五线谱（两行谱表系统）" },
        pros: {
          en: ["The universal language: gives you access to 500 years of global literature", "Spatial density: 50 measures can be seen and analyzed on a single paper page", "Reveals harmonic architecture, voice-leading, and phrasing at a glance"],
          zh: ["通行全球的音乐母语：一举解锁人类500年来的浩瀚乐谱宝库", "高维信息压缩：一张A4纸即可纵览数十小节的和声脉络与织体", "一眼看清和声走向、声部对位、强弱呼吸与作曲家原生指法"]
        },
        cons: {
          en: ["Steep initial friction: reading notes feels like deciphering ancient hieroglyphics for the first 30 days", "Requires deliberate flashcard and landmark-note practice"],
          zh: ["初期学习摩擦较大：前一个月看低音谱表如读天书，需要耐受进展缓慢的焦躁", "需要配合地标音法与闪卡做刻意的视觉-神经反射训练"]
        }
      },
      verdict: {
        en: "Synthesia is a transient crutch. Use it for 5 minutes only to clarify awkward physical hand collisions, then immediately return to the paper score. Invest the effort into notation early; your future self will thank you endlessly.",
        zh: "裁决建议：下落瀑布流只能作为临时救急的脚手架。仅在遇到极别扭的跨手或找不准键位时看上几分钟辅助定位，随后果断切回乐谱。尽早咬牙攻克五线谱，这是成人通往真正自学自由的唯一大道。"
      }
    },
    {
      id: "gamified-vs-method-books",
      topic: {
        en: "Gamified Mobile Apps (Simply Piano) vs. Structured Adult Method Books (Alfred/Faber)",
        zh: "游戏化自学App（Simply Piano） 对决 严肃经典成人教程（阿尔弗莱德/菲伯尔）"
      },
      overview: {
        en: "Can software replace pedagogical structure, or does it substitute external ceremony for internal cognitive updates?",
        zh: "交互软件能否取代系统教学法，还是用外在打卡仪式取代了内在认知突触的深度更新？"
      },
      left: {
        title: { en: "Gamified Apps (Simply Piano / Yousician)", zh: "游戏化打卡App（Simply Piano等）" },
        pros: {
          en: ["Addictive daily streak mechanisms prevent quitting in week 1", "Fun backing tracks disguise the awkwardness of early beginner sound", "Microphone/MIDI feedback confirms whether you hit the right pitch"],
          zh: ["精心设计的多巴胺连击与打卡机制，极大降低首周放弃率", "好听的伴奏音轨掩盖了初学击键的干瘪刺耳，保护演奏热情", "麦克风/MIDI自动判定音高对错，给予即时确认反馈"]
        },
        cons: {
          en: ["Bypasses sound quality, wrist suppleness, and arm weight entirely", "Creates app dependency: students freeze when presented with real sheet music", "Expensive recurring monthly/annual subscription fees"],
          zh: ["对触键音色、手腕柔韧绕圈与手臂自然沉落完全无感，极易练出死力", "产生严重的软件依赖：一旦拿到没有光标滚动的标准纸谱便寸步难行", "按月或按年持续扣费，长期使用成本并不低"]
        }
      },
      right: {
        title: { en: "Adult Method Books (Alfred / Faber)", zh: "系统化成人教程（Alfred / Faber）" },
        pros: {
          en: ["Designed by master pedagogues over decades of trial and refinement", "Cultivates real eyes-on-the-score habits from lesson one", "Inexpensive one-time physical purchase; portable and permanent"],
          zh: ["凝聚数十年全球钢琴教育泰斗的教学精华，循序渐进无断层", "从第一课起培养看谱不看手的盲弹空间感与和声骨架理解", "极低廉的一次性购书成本，终身可读，随时用铅笔做个性化标注"]
        },
        cons: {
          en: ["Requires self-discipline without flashing digital dopamine animations", "Repertoire can feel vintage or overly dry if not supplemented with real favorites"],
          zh: ["没有数字界面的声光多巴胺刺激，全凭自学者的内在原动力驱动", "书内收录的部分老旧民谣可能不符合现代年轻成人的审美偏好"]
        }
      },
      verdict: {
        en: "Do not let an algorithm dictate your relationship with music. Buy an acclaimed method book like Alfred or Faber as your spinal curriculum, and treat apps merely as optional supplemental toys.",
        zh: "裁决建议：切莫让算法和游戏光标绑架你的音乐心灵。将阿尔弗莱德或菲伯尔教材作为不可动摇的主轴脊梁，将各类App仅视为偶尔调剂或视奏测验的辅助工具。"
      }
    },
    {
      id: "selftaught-vs-teacher",
      topic: {
        en: "Self-Directed Study with Modern Tools vs. Weekly Private In-Person Teacher",
        zh: "基于现代数字工具的纯自主研习 对决 传统每周线下私教"
      },
      overview: {
        en: "Evaluating pacing autonomy, financial cost, and physical ergonomics feedback.",
        zh: "在学习节奏自主性、经济时间成本与实体触觉体态纠偏之间的权衡。"
      },
      left: {
        title: { en: "Autonomous Self-Directed Study", zh: "现代全自主研习模式" },
        pros: {
          en: ["Total freedom: practice when you want, for as long as you want", "Zero commute time or scheduling anxiety for busy adult professionals", "100% control over repertoire: learn only pieces you genuinely love"],
          zh: ["绝对的作息自由：随时上琴练琴，完美适配成人的高负荷碎片生活", "零通勤成本与日程焦虑，免去每周固定时间赴约的沉重心理负担", "100%曲目自主权：只弹自己真正深爱动心的作品，绝不受教条强迫"]
        },
        cons: {
          en: ["Risk of developing chronic physical tension without knowing it", "Self-evaluation blindness: hard to hear your own rhythmic fluctuations while playing"],
          zh: ["若缺乏自省，可能长期存在耸肩、压腕、折指等不良发力恶习而不自知", "演奏时因全神贯注而难以客观听清自己的节拍微颤与强弱失衡"]
        }
      },
      right: {
        title: { en: "Weekly Private In-Person Teacher", zh: "每周线下专业私教" },
        pros: {
          en: ["Physical tactile intervention: a great teacher gently touches your wrist/shoulder to instantly release hidden tension", "External accountability prevents prolonged practice droughts", "Tailored pedagogical diagnosis on tricky polyphonic fingering"],
          zh: ["实体触觉即时干预：优秀名师轻触你的手腕与肩头，能瞬间点醒你的潜意识紧张", "强外部契约与监督感，有效阻止拖延症与漫长的练琴荒废期", "针对刁钻小节指法，能当面传授最贴合你手掌骨骼生理的最佳方案"]
        },
        cons: {
          en: ["High ongoing cost ($60–$120/hr, $3,000–$6,000 annually)", "Bad/rigid teachers often treat adults like four-year-olds, forcing nursery rhymes and killing intrinsic passion"],
          zh: ["持续经济支出高昂（每小时数十至上百欧/美元，年均数千成本）", "良莠不齐：平庸或死板的老师常把成年人当四岁幼童教训，死抠枯燥音阶，彻底扼杀成年人珍贵的音乐热情"]
        }
      },
      verdict: {
        en: "Self-direction with video recording is exceptionally powerful for adults. If possible, adopt a hybrid model: study independently using curated tools, and book a single consultation lesson once every 2-3 months solely for an ergonomic and posture audit.",
        zh: "裁决建议：对于具备高度自省力的成年人，自主学习配合手机侧面录像自查具有极高上限。最佳方案是“以自主探索为主轴，每季度约请高水平名师上一次问诊课”，单刀直入排查体态张力。"
      }
    },
    {
      id: "digital-vs-acoustic",
      topic: {
        en: "88-Key Weighted Digital Piano vs. Traditional Acoustic Upright Piano",
        zh: "88键重锤电钢琴 对决 传统原声机械立式钢琴"
      },
      overview: {
        en: "Comparing physical hammer mechanics, living acoustic resonance, and domestic reality.",
        zh: "在真实击弦木质共鸣、物理杠杆触感与家庭现实生活便利性之间的决断。"
      },
      left: {
        title: { en: "Weighted Digital Piano (Roland/Yamaha)", zh: "88键高品质重锤电钢琴" },
        pros: {
          en: ["Silent headphone practice allows deep concentration at 6 AM or 11 PM", "Never needs tuning (saving $200–$400 annually in climate fluctuations)", "Lightweight, easy to move, flawless USB-MIDI recording connectivity"],
          zh: ["耳机静音功能让你在清晨6点或深夜11点心无旁骛沉浸弹奏，不扰任何人", "终生无需调律，无惧北方暖气与潮湿季的音准走调（年省数千元维护费）", "重量适中搬运自由，自带高精度MIDI接口直连平板电脑与软件"]
        },
        cons: {
          en: ["Lacks the full, visceral body resonance of physical wood vibrating in an open room", "Key action, while weighted, is a simulated lever rather than a real felt hammer striking steel wire"],
          zh: ["缺少整个实木琴腔与铸铁板在真实空气中掀起的三维空间物理震颤", "击弦手感即便有重锤模拟，终究是弹簧与配重块杠杆，与真琴微弱的擒纵齿轮感略有偏差"]
        }
      },
      right: {
        title: { en: "Acoustic Upright Piano", zh: "传统原声机械钢琴（立式/三角）" },
        pros: {
          en: ["Incomparable sensory richness: feeling the vibration through the keybed into your fingertips and chest", "Teaches microscopic control over hammer speed, velocity, and tone color", "A durable acoustic piano can last multiple generations"],
          zh: ["无可替代的感官震撼：指尖按下的瞬间，琴弦震动通过琴凳直抵胸膛", "倒逼指尖建立对琴槌撞击琴弦微秒级速度的敏锐控制力，音色层次极其丰润", "保养得当的优质机械钢琴可作为家族传家宝，陪伴一生"]
        },
        cons: {
          en: ["Cannot be muted: practicing an awkward 2-bar loop 50 times will test the patience of family and neighbors", "Heavy (200-250kg), sensitive to humidity, requires regular professional tuning"],
          zh: ["无法完全静音：把一个卡壳小节慢练50遍的声响，将严酷考验家人与邻居的包容底线", "自重极沉（200公斤以上），对温湿度极其敏感，每年须定期调音保养"]
        }
      },
      verdict: {
        en: "For 90% of adult learners, an 88-key weighted digital piano (like Roland FP-30X or Yamaha P-225) is the vastly superior strategic choice. The psychological freedom to make mistakes in headphones without being heard dramatically accelerates adult experimentation.",
        zh: "裁决建议：对90%的成年初学者而言，一台88键优质重锤电钢是更理性的战略首选。“戴上耳机不怕任何人听见我弹错”的心理安全感，能让成人完全放下包袱、大胆反复试错，学习效率呈倍数提升。"
      }
    },
    {
      id: "blocked-vs-deliberate-practice",
      topic: {
        en: "Mindless Repetition (Playing Through) vs. Deliberate Micro-Chunking",
        zh: "从头弹到尾的机械盲刷 对决 微观切片与慢速刻意练习"
      },
      overview: {
        en: "Why playing a piece start-to-finish 10 times gives the illusion of work while producing zero neurological improvement.",
        zh: "为什么把整首曲子从头到尾流水账般弹10遍，只会制造虚假努力的幻觉，而无法带来真正的神经重塑？"
      },
      left: {
        title: { en: "Mindless 'Play-Through' Practice", zh: "从头到尾机械通弹（流水账模式）" },
        pros: {
          en: ["Feels emotionally pleasant to hear the parts you already know", "Requires low mental effort"],
          zh: ["心理体验舒适：反复沉醉在自己已经熟练的段落中获得虚荣满足", "几乎不消耗心智算力，大脑处于被动的半睡眠状态"]
        },
        cons: {
          en: ["Every time you reach a hard measure, you stumble, stop, and guess — actively cementing mistakes into muscle memory", "Takes 10x longer to learn pieces; perpetual performance anxiety at specific bars"],
          zh: ["每次弹到难点处都会卡壳、停顿、重来——等于在主动给大脑强化“错误记忆轨迹”", "耗费十倍时间却原地踏步；一到特定小节就心虚手抖，永远不敢在他人面前完整演奏"]
        }
      },
      right: {
        title: { en: "Deliberate Micro-Chunking (+1 Friction)", zh: "微观切片与慢速刻意练习（+1摩擦）" },
        pros: {
          en: ["Isolates the single friction point (1-2 beats, or a specific thumb-under motion)", "Practices ultra-slow with 100% accuracy, creating pristine neural myelin sheaths", "Saves hundreds of hours; turns difficult passages into effortless automatic reflex"],
          zh: ["极其精准地孤立摩擦点（甚至只切出1个节拍或单次大拇指穿指动作）", "在零错音的超慢速度下做足手腕轨迹雕琢，让大脑一次性镀上高纯度的神经髓鞘", "省去数百小时无效耗损；让曾经看似天堑的难关变成毫无压力的肌肉自然反射"]
        },
        cons: {
          en: ["Requires intense concentration and mental discipline", "Doesn't 'sound like pretty music' for the first 15 minutes of a practice session"],
          zh: ["需要高度清醒的心智算力投入与克制力，前15分钟枯燥切片时“听起来不像在弹好听的曲子”", "需要成年人勇敢直面自己指尖笨拙的真实落差"]
        }
      },
      verdict: {
        en: "The golden rule of piano: Never practice a mistake. If you stumble twice in the same spot, immediately STOP. Drop the tempo by 50%, isolate just the 4 problematic notes, and play them perfectly 5 times in slow motion. Speed is merely the consequence of accuracy.",
        zh: "裁决建议：钢琴研习的铁律是“永远不要重复一次错误”。如果同一个位置连续卡壳两次，立刻紧急制动！将速度压低一半，仅切出导致卡壳的4个音符，在慢动作中完美弹奏5遍再顺滑缝合。速度从来不是赶出来的，速度只是极度精准的自然副产物。"
      }
    }
  ],

  // Practice Architecture & Neuroplasticity
  practiceArchitecture: {
    hero: {
      en: {
        title: "Rewiring the Adult Motor Cortex: The 4-Stage Practice Cycle",
        subtitle: "How adult learners build clean, indelible muscle memory through deliberate feedback loops."
      },
      zh: {
        title: "重塑成年人运动皮层：四阶闭环练习架构",
        subtitle: "成年自学者如何通过高觉察反馈回路，建立干净、牢不可破的指尖肌肉记忆。"
      }
    },
    cycleStages: [
      {
        step: "01",
        name: { en: "Deconstruction & Mapping", zh: "第一步：谱面解构与解剖测绘" },
        desc: {
          en: "Before touching a single key, sit with a pencil. Identify key signatures, harmonic anchors, bass note trajectories, and repetitive chord progressions. Mark awkward fingerings. Clap the rhythm with hands on knees to ensure your brain understands the pulse before your fingers are tasked with striking keys.",
          zh: "在手指触碰琴键之前，先手持铅笔静坐读谱。标出调号、调性转折、和弦骨架与低音线条走向。圈出容易犯错的换指处。双手在膝盖上拍打切分节奏，确保大脑在命令手指击键前，早已吃透了节奏的起伏节拍。"
        }
      },
      {
        step: "02",
        name: { en: "Micro-Chunking (Slicing)", zh: "第二步：微观切片与声部剥离" },
        desc: {
          en: "Never attempt to swallow a 4-page piece whole. Slice the piece into digestible chunks of 1 to 2 measures, or even single transition leaps. Practice Hands Separate (HS) first to allow each hand to establish an unburdened physical trajectory, then merge hands at an agonizingly slow tempo.",
          zh: "切忌贪大求全通篇吞咽。将乐曲无情切割为1到2小节的极小微观切片，甚至仅切出一次跨八度的大跳。先严格单手练习（HS），让每只手臂摸清自如的发力轨迹，随后在极慢速度下将双手如同齿轮咬合般精准合并。"
        }
      },
      {
        step: "03",
        name: { en: "Ultra-Slow Frictionless Execution", zh: "第三步：超低速零误差刻画" },
        desc: {
          en: "Play at 30% to 50% of the target tempo. At this speed, mistakes are strictly forbidden. Drop all tension from your neck, shoulders, and wrists on every downbeat. If you feel tightness in your tendons, pause immediately, exhale, and feel gravity pulling your elbows down. Speed is never rushed; it emerges as a byproduct of zero tension.",
          zh: "以目标速度的30%至50%龟速慢练。在此速度下，严禁出现任何错音或迟疑。在每一个强拍上主动检查并释放肩部、颈部与手腕的多余紧绷。若感到小臂肌腱发酸，立即悬停悬浮、深呼吸，感受重力将手肘自然下拉。速度永非硬赶，而是在松弛中自然流淌。"
        }
      },
      {
        step: "04",
        name: { en: "Interleaving & Sleep Consolidation (PPI)", zh: "第四步：交叉轮转与夜间睡眠沉淀" },
        desc: {
          en: "Do not grind the same chunk for 60 minutes straight. After 10 focused minutes, switch to an entirely different section or piece (interleaved practice). When you go to sleep, your brain’s motor cortex replays these deliberate loops during slow-wave sleep, cementing synapses through Post-Practice Improvement (PPI). Tomorrow, your fingers will know what baffled them today.",
          zh: "切勿对着同一小节死磕一小时。高强度聚焦攻克10分钟后，果断切换至完全不同风格的段落或乐曲（交叉轮换练习）。当你安然入睡时，大脑运动皮层会在慢波睡眠期间对白天的高质量回路进行高保真重放与髓鞘加固（PPI效应）。次日清晨，双手自然会流畅弹出昨日卡壳的旋律。"
        }
      }
    ],
    blueprints: [
      {
        time: "20 min",
        title: { en: "The Busy Professional's Daily Maintenance Loop", zh: "极忙工作日：20分钟敏捷维稳微循环" },
        schedule: [
          { duration: "3 min", activity: { en: "Physical wrist circles & 1 scale/arpeggio warm-up (deep arm weight)", zh: "手腕柔韧绕圈操 + 单条音阶琶音极慢热身（体会重量落底）" } },
          { duration: "12 min", activity: { en: "Surgical strike on exactly ONE tricky 2-bar chunk of your current piece", zh: "精准外科手术：只攻坚当前核心曲目中某处卡壳的2小节切片" } },
          { duration: "5 min", activity: { en: "Full playthrough of an old familiar favorite purely for joy and connection", zh: "从头到尾完整温习一首已熟稔的旧曲，单纯享受指尖流淌与音乐共鸣" } }
        ]
      },
      {
        time: "45 min",
        title: { en: "The Standard Growth Routine (Balanced Mastery)", zh: "标准进阶日：45分钟全维均衡研习架构" },
        schedule: [
          { duration: "7 min", activity: { en: "Sight-reading drill (Piano Marvel SASR or 1 Bartók Mikrokosmos piece)", zh: "视奏敏捷度激活（Piano Marvel视奏测验 或 攻克1首巴托克小宇宙）" } },
          { duration: "8 min", activity: { en: "Technique/Tone sculpting (Hanon with arm rotation or Burgmüller etude)", zh: "纯触键发力雕琢（结合前臂旋转的哈农 或 布格缪勒浪漫小练习曲）" } },
          { duration: "25 min", activity: { en: "Core project piece: micro-chunking new sections, hands separate then together", zh: "核心攻坚大曲：拆解新段落切片，先严格单手后微慢速双手拼合" } },
          { duration: "5 min", activity: { en: "Mental playback with closed eyes: reviewing the score away from the keys", zh: "闭目冥想内听：脱离琴键在脑海中无声过谱，排查指法记忆盲点" } }
        ]
      },
      {
        time: "60+ min",
        title: { en: "The Weekend Deep Immersion Session", zh: "周末深度沉浸：60分钟以上全景音乐探索" },
        schedule: [
          { duration: "10 min", activity: { en: "Ear training & harmonic analysis (Functional Ear Trainer or chord decoding)", zh: "听觉引力与和声解剖（Functional Ear Trainer 或 流行曲和弦听辨）" } },
          { duration: "15 min", activity: { en: "Technique & Polyphony (Bach Two-Part Invention or Little Prelude hands-separate)", zh: "复调声部对位训练（巴赫小前奏曲 或 二部创意曲单声部精修）" } },
          { duration: "25 min", activity: { en: "Deep work on major repertoire piece (Passacaglia / Forrest Gump) with metronome", zh: "主攻大作精细雕琢（帕萨卡利亚/阿甘正传），节拍器微调递增提速" } },
          { duration: "10 min", activity: { en: "Improvisation / Creative exploration: jamming over a simple left-hand bass pattern", zh: "即兴释放与自由漫步：在简单的左手五度循环上，右手随心涂抹即兴旋律" } }
        ]
      }
    ]
  },

  // Repertoire Ladder (5 Levels with Real Adult Favorites)
  repertoire: [
    {
      level: 1,
      title: { en: "Level 1: Grounding & Landmark Literacy", zh: "阶梯一：调性扎根与键盘地标盲弹" },
      subtitle: { en: "Establishing bilateral hand coordinates without staring anxiously at your fingers.", zh: "建立左右手物理坐标感，克服低头死盯手指的焦虑。" },
      pieces: [
        {
          name: "Minuet in G major (BWV Anh. 114)",
          composer: "Christian Petzold / J.S. Bach Notebook",
          focus: { en: "Hand positioning, light wrist bounces on quarter notes, clean finger transitions.", zh: "高低音声部清晰交替，落滚手腕起伏，跳音与连音的触键对比。" },
          tips: { en: "Keep the left-hand quarter notes buoyant and quiet so the right hand's famous melody sparkles.", zh: "左手四分音符伴奏务必保持弹性与微弱力度，切忌喧宾夺主压过右手名旋律。" }
        },
        {
          name: "Ode to Joy (Themes & Chords)",
          composer: "L. van Beethoven (Arranged)",
          focus: { en: "Solid I, IV, V7 left-hand chord changes underneath a singing right-hand melodic line.", zh: "左手I级、IV级、V7级柱式和弦沉稳换位，右手歌唱性旋律线条。" },
          tips: { en: "Change chords with minimal vertical hand movement — glide smoothly between inversions.", zh: "换和弦时手掌尽量贴近键面滑行，杜绝从高空盲目砸键的粗糙发力。" }
        }
      ]
    },
    {
      level: 2,
      title: { en: "Level 2: Cohesion, Polyphony & Expressive Breathe", zh: "阶梯二：声部呼吸、触键对比与初阶复调" },
      subtitle: { en: "Separating dynamics: making one hand whisper while the other sings warmly.", zh: "声部分层：学会让一只手低声呢喃，另一只手温润高歌。" },
      pieces: [
        {
          name: "Gymnopédie No. 1",
          composer: "Erik Satie",
          focus: { en: "Large left-hand leaps (bass note to high chord), delicate atmospheric half-pedaling.", zh: "左手宽幅大跳（低音根音跃迁至中音区柔和弦），空灵纯净的踏板微调换音。" },
          tips: { en: "Do not rush the left-hand leap. Float your wrist upward, move across horizontally, and land softly on the chord cushion.", zh: "左手起跳切忌慌乱。手腕轻柔向上带起，在空中划出平滑弧线，轻柔落在和弦琴键上。" }
        },
        {
          name: "Arabesque Op. 100 No. 2",
          composer: "Friedrich Burgmüller",
          focus: { en: "Rapid five-finger fluidity, snappy two-note slurs, rhythmic drive.", zh: "五指快速跑动颗粒感、两音连线呼吸感与明快的戏剧驱动力。" },
          tips: { en: "Practice in dotted rhythms (long-short, short-long) to even out weak 4th and 5th fingers.", zh: "用附点长短变节奏强化无名指与小指，确保上行跑音如珍珠般颗颗分明。" }
        }
      ]
    },
    {
      level: 3,
      title: { en: "Level 3: Expressive Lyricism & Cinematic Drama", zh: "阶梯三：影视金曲抒情、旋律揉弦与民谣织体" },
      subtitle: { en: "Bridging classical discipline with beloved contemporary emotion.", zh: "将严谨的古典触键纪律，融入刻骨铭心的现代影视与流行旋律。" },
      pieces: [
        {
          name: "Chengdu (成都) — Piano Arrangement",
          composer: "Zhao Lei (Arr. for Solo Piano)",
          focus: { en: "Tender arpeggiated left hand, vocal rubato phrasing, delicate dynamic shading.", zh: "质朴感人的左手九度与十度分解琶音，如吟如诉的自由速度（Rubato）处理。" },
          tips: { en: "Sing the lyrics aloud while playing the melody. Let the left-hand bass notes resonate like warm footsteps on Yulin Road.", zh: "弹奏时心中默念歌词呼吸。让低音根音如漫步玉林路的脚步般沉稳深远，切忌生硬敲击。" }
        },
        {
          name: "Game of Thrones (Main Theme)",
          composer: "Ramin Djawadi",
          focus: { en: "Syncopated 6/8 rhythmic pulse, driving cello-like left-hand ostinato, epic escalation.", zh: "荡气回肠的6/8拍大提琴律动，顽固低音（Ostinato）层层推进的史诗张力。" },
          tips: { en: "Maintain an unwavering metronomic bedrock in your left hand while shaping the dynamic crescendo from p to fff.", zh: "左手如同铁军战鼓般坚不可摧，右手主题从弱奏（p）层层蓄力爆发至极强（fff）。" }
        },
        {
          name: "Prelude in E minor Op. 28 No. 4",
          composer: "Frédéric Chopin",
          focus: { en: "Microsopic left-hand chromatic voicing changes, immense emotional restraint, cantabile touch.", zh: "左手极其幽微的半音和声缓慢下沉，无尽深沉的克制哀婉，如泣如歌触键。" },
          tips: { en: "Play the left-hand pulsing chords at pianissimo (pp) — treat each moving inner voice like a subtle shadow shift.", zh: "左手持续伴奏音务必压至极弱，把每一个内声部半音的移动当作光影的微妙倾斜。" }
        }
      ]
    },
    {
      level: 4,
      title: { en: "Level 4: Fluidity, Wide Arpeggios & Pure Touch", zh: "阶梯四：长琶音奔流、长音共鸣与如水诗意" },
      subtitle: { en: "Mastering large hand movements and seamless continuous pedaling.", zh: "驾驭长线条宽幅跑动，踏板如同呼吸般纯净无瑕。" },
      pieces: [
        {
          name: "Forrest Gump Suite (Feather Theme)",
          composer: "Alan Silvestri",
          focus: { en: "Delicate cascading high-register triplets, buoyant rising arpeggios, feather-light release.", zh: "高音区轻盈如羽毛般飘零的三连音，层层上涌的分解琶音，极致纯净的高音触键。" },
          tips: { en: "The opening high notes must be played without harshness: let the arm weight drop from above with cushion-soft fingertips.", zh: "开篇的高音绝不可敲砸：指尖肉垫柔软着陆，手臂自然下沉，让余音在空气中缓缓荡漾开来。" }
        },
        {
          name: "Nuvole Bianche",
          composer: "Ludovico Einaudi",
          focus: { en: "Endurance, hypnotic rhythmic loops, four-note polyrhythmic layer balance.", zh: "持久专注力、极简主义催眠般的和声循环与左右手声部层次分离。" },
          tips: {
            en: "Avoid forearm strain during repetitive patterns by constantly rotating the wrist in micro-ovals. (Study Companion: Consult Michael Maiber's phrase-by-phrase YouTube tutorials — the author learned virtually all of his Einaudi repertoire from this channel through persistent personal struggle).",
            zh: "面对长时间重复跑动的和声模式，手腕必须保持微型椭圆晃动，切断一切机械静止僵化。（研习推荐：参考 YouTube 导师 Michael Maiber 的逐句慢拆教学——本指南作者几乎所有的艾诺迪作品，都是对照该频道的慢速拆解并结合自身不断死磕摸索而攻克的）。"
          }
        }
      ]
    },
    {
      level: 5,
      title: { en: "Level 5: Grand Architecture, Counterpoint & Virtuosity", zh: "阶梯五：宏大复调建筑、变奏狂潮与演奏级挑战" },
      subtitle: { en: "The summit of adult perseverance: multifaceted polyphonic mastery.", zh: "成人毅力与智慧的壮丽峰峦：在键盘上构筑多声部交响殿堂。" },
      pieces: [
        {
          name: "Passacaglia (Theme & Variations)",
          composer: "G.F. Handel / arr. Johan Halvorsen",
          focus: { en: "Stately 8-bar ground bass (Chaconne progression), explosive running sixteenth notes, dramatic chordal climaxes.", zh: "威严庄重的固定低音骨架（恰空下行织体），暴风雨般的十六分音符交替奔流，波澜壮阔的交响和弦高潮。" },
          tips: { en: "The piece is an architectural monument. Never rush variation transitions. Memorize the harmonic skeleton (G min - C min - D - G min) so your fingers always know their home base.", zh: "这是一座巴洛克大理石建筑。变奏切换时节拍框架坚若磐石。死死咬住底层和声骨架，即便双手快速换位，心智原点也稳如泰山。" }
        },
        {
          name: "Nocturne in E-flat major Op. 9 No. 2",
          composer: "Frédéric Chopin",
          focus: { en: "Waltz left-hand jumps (bass note to two mid-register chords), florid bel-canto coloratura runs.", zh: "经典摇曳圆舞曲低音大跳，宛如美声歌剧花腔女高音般华丽多变的装饰音跑动。" },
          tips: { en: "Treat the right-hand fiorituras (11-note, 22-note runs) as unmeasured liquid ribbons: let them float freely over the strict left-hand heartbeat.", zh: "将右手华丽的十一连音、二十二连音视作流淌的丝带：左手伴奏如同从容的心跳，右手旋律则在空中自由翱翔。" }
        }
      ]
    }
  ],

  // Interactive Toolkit Matcher Questions & Algorithm
  matcherQuiz: {
    questions: [
      {
        id: "q1_goal",
        text: {
          en: "What is your primary musical aspiration at this stage?",
          zh: "在这个阶段，你最迫切、最渴望实现的音乐心愿是什么？"
        },
        options: [
          {
            key: "pop_soundtrack",
            text: {
              en: "Play beloved film scores & pop songs (e.g. Forrest Gump, Chengdu, Game of Thrones)",
              zh: "弹奏心爱的影视配乐与流行金曲（如《阿甘正传》《成都》《权力的游戏》等）"
            }
          },
          {
            key: "classical_literacy",
            text: {
              en: "Master classical piano repertoire & deep sheet literacy (Bach, Chopin, Handel)",
              zh: "研习正统古典杰作与深度识谱体系（巴赫复调、肖邦抒情、亨德尔变奏）"
            }
          },
          {
            key: "chords_improv",
            text: {
              en: "Play by ear, understand chord progressions, and accompany singing/family",
              zh: "掌握和弦走向、即兴弹唱，为家人伴奏或听歌即兴配弹"
            }
          }
        ]
      },
      {
        id: "q2_time",
        text: {
          en: "How much focused, undisturbed practice time can you protect on typical days?",
          zh: "在绝大多数普通工作日，你能够雷打不动守住多少专注练琴时间？"
        },
        options: [
          {
            key: "time_15_20",
            text: {
              en: "15 to 20 minutes (busy schedule, need high-density micro-chunks)",
              zh: "15 到 20 分钟（生活节奏极快，需要高密度微观切片与外科手术式练琴）"
            }
          },
          {
            key: "time_30_45",
            text: {
              en: "30 to 45 minutes (the sweet spot for steady, compounding progression)",
              zh: "30 到 45 分钟（稳健推进黄金期，兼顾基本功、新曲拆解与旧曲温习）"
            }
          },
          {
            key: "time_60_plus",
            text: {
              en: "60 minutes or more (deep weekend immersion, rapid multi-track growth)",
              zh: "60 分钟以上（深度沉浸期，可全维推进复调、练耳、即兴与大型乐曲）"
            }
          }
        ]
      },
      {
        id: "q3_setup",
        text: {
          en: "What is your current keyboard / piano hardware reality?",
          zh: "你目前的琴键硬件环境处于哪种状态？"
        },
        options: [
          {
            key: "no_piano_or_spring",
            text: {
              en: "No piano yet, or a 61-key unweighted plastic keyboard (need upgrade guidance)",
              zh: "尚未购琴，或只有一台61键塑料弹簧无重锤电子琴（急需升级建议）"
            }
          },
          {
            key: "weighted_digital",
            text: {
              en: "88-key weighted hammer action digital piano (Roland, Yamaha, Kawai) with headphones",
              zh: "已拥有88键全配重重锤电钢琴（Roland/Yamaha/Kawai等）且支持耳机练习"
            }
          },
          {
            key: "acoustic_piano",
            text: {
              en: "Traditional acoustic upright or grand piano in my home",
              zh: "家中已配备传统机械立式或三角钢琴"
            }
          }
        ]
      },
      {
        id: "q4_bottleneck",
        text: {
          en: "What currently represents your greatest friction point or inner obstacle?",
          zh: "目前在练琴过程中，最让你感到卡壳或挫败的核心瓶颈是什么？"
        },
        options: [
          {
            key: "reading_notes",
            text: {
              en: "Deciphering sheet music (especially bass clef) feels painfully slow",
              zh: "看谱识谱极其缓慢（尤其是低音谱表和加线音符），如同破译密码"
            }
          },
          {
            key: "coordination_tension",
            text: {
              en: "Left and right hands fight each other; forearm or wrist feels tense after 20 minutes",
              zh: "双手不听使唤、各顾各的；弹奏20分钟以上小臂外侧发酸、手腕僵硬"
            }
          },
          {
            key: "consistency_doubts",
            text: {
              en: "Losing motivation, wondering if 'I started too late' to ever sound genuinely good",
              zh: "缺乏确定感，偶尔怀疑自己“是不是起步太晚了，注定弹不好听”而动摇心志"
            }
          }
        ]
      }
    ]
  },

  // Resources I Enjoy & Aesthetic Inspirations
  enjoyedResources: [
    {
      id: "katherine-cordova",
      name: "Katherine Cordova",
      channel: "@KatherineCordova",
      role: {
        en: "Self-Taught Canadian Pianist, Arranger & Expressive Soloist",
        zh: "加拿大自学成才钢琴家、编曲人与极富感染力的独奏家"
      },
      tags: ["Self-Taught", "Rock & Film Piano", "Dynamic Nuance", "Aesthetic Resonance"],
      link: "https://www.youtube.com/@KatherineCordova",
      description: {
        en: "A phenomenal self-taught Canadian pianist whose acoustic grand arrangements of rock, film soundtracks, and modern masterpieces (Queen, Radiohead, Hans Zimmer, Linkin Park, Evanescence) capture breathtaking emotional gravity, visceral dynamics, and cinematic sweep.",
        zh: "来自加拿大的天才级自学成才钢琴家。她将摇滚史诗、电影原声与现代金曲（Queen、Radiohead、Hans Zimmer、Linkin Park等）改编为纯三角钢琴独奏，琴声中蕴含着令人屏息的情感张力、宏阔的动态对比与如电影长镜头般的叙事意境。"
      },
      whyIEnjoy: {
        en: "Her playing is pure musicality without an ounce of sterile conservatory pedantry. Watching her control of whisper-quiet pianissimo and surging, thunderous fortissimo demonstrates what adult piano playing can be: deeply personal, intensely expressive, and entirely unconstrained by academic dogma. It is one of my favorite sanctuaries to find inspiration and fall in love with the sound of the piano all over again.",
        zh: "她的演奏是纯粹的音乐性表达，毫无学院派刻板教条的匠气。观摩她在极轻弱音（pp）与雷霆强音（ff）之间的自如呼吸，向所有成人学琴者展示了钢琴的真谛：极其私密、极其热烈，且完全不受传统条条框框的束缚。这是我最常驻足、也是最能让我重新燃起练琴炽热爱意的精神圣殿。"
      },
      highlights: {
        en: "Breathtaking arrangements of Bohemian Rhapsody, Creep, Time (Inception), and Bring Me To Life; phenomenal left-hand bass authority and polyphonic voice separation.",
        zh: "改编的《波西米亚狂想曲》《Creep》《盗梦空间 Time》《Bring Me To Life》等名作令人叹为观止；低音掌控力与各声部音色分离度极具辨识度。"
      },
      takeaway: {
        en: "You don't need a 15-year traditional conservatory pedigree to touch people's hearts at the keyboard. True musical voice comes from deep listening, emotional honesty, and voluntary dedication.",
        zh: "打动人心不需要十五年的学院体制履历。真正的音乐之声，源于深刻的内听、诚实的情感投射，以及完全出于自愿的热爱。"
      }
    },
    {
      id: "michael-maiber",
      name: "Michael Maiber",
      channel: "@MichaelMaiber",
      role: {
        en: "Self-Taught Pianist & Phrase-by-Phrase Mentor",
        zh: "自学成才钢琴博主、逐句慢拆实战领航员"
      },
      tags: ["Ludovico Einaudi", "Step-by-Step", "Self-Taught Path", "Endless Patience"],
      link: "https://www.youtube.com/@MichaelMaiber",
      description: {
        en: "The dedicated channel where I learned virtually all of my Ludovico Einaudi repertoire (Nuvole Bianche, Divenire, Una Mattina, Experience) alongside my own daily struggling.",
        zh: "陪伴我啃下几乎全部鲁多维科·艾诺迪（Ludovico Einaudi）曲目（《Nuvole Bianche》《Divenire》《Una Mattina》《Experience》等）的挚爱宝藏频道。"
      },
      whyIEnjoy: {
        en: "Michi taught himself to play and read music via YouTube, which gives his tutorials an innate empathy for where self-taught adults get stuck. He breaks down intimidating rolling arpeggios and rhythmic syncopations into calm, bite-sized 2-measure phrases.",
        zh: "Michi 本人同样是通过 YouTube 自学会了弹琴与读谱，因此他的教学对自学者在哪个小节、哪双手上卡壳有着极敏锐的共情。他把令人望而生畏的长琶音与跨声部切分拆解成平和、极易下咽的2小节切片。"
      },
      highlights: {
        en: "Complete, phrase-by-phrase walkthroughs of the Einaudi canon, Yann Tiersen (Amélie), and lyrical Chopin melodies with slow hands-separate demonstrations.",
        zh: "艾诺迪全套代表作、扬·提尔森（《天使爱美丽》）与肖邦抒情曲目的完整逐句慢速精拆教学，配有清晰的分手慢动作示范。"
      },
      takeaway: {
        en: "When a piece feels impossibly long or complex, patient phrase-by-phrase deconstruction paired with persistent personal struggle will eventually conquer every single measure.",
        zh: "面对看似长篇浩繁的乐曲，只要借助有耐心的慢速分句拆解，再加上自己不怕枯燥的死磕与坚持，没有任何一个小节是无法攻克的。"
      }
    },
    {
      id: "ludovico-einaudi",
      name: "Ludovico Einaudi",
      channel: "@ludovicoeinaudi",
      role: {
        en: "Contemporary Minimalist Composer & Pianist",
        zh: "当代新古典极简主义作曲大师、钢琴家"
      },
      tags: ["Minimalism", "Harmonic Ostinato", "Soulful Restraint", "Repertoire Heart"],
      link: "https://www.youtube.com/@ludovicoeinaudi",
      description: {
        en: "The living titan of modern neoclassical piano. His cyclical ostinatos, expansive pedal resonances, and crystalline melodies have defined the emotional soundtrack of modern life.",
        zh: "当代新古典钢琴界的泰斗级大师。他循环往复的固定低音、旷远空灵的踏板延音与晶莹剔透的旋律线，构筑了现代人心灵深处的情感共鸣场。"
      },
      whyIEnjoy: {
        en: "Einaudi's music is the ultimate rebuttal to the myth that piano music must be blisteringly virtuosic to be meaningful. The genius lies in restraint: four chords repeated with subtle touch variations that evoke an entire universe of longing and peace.",
        zh: "艾诺迪的音乐是对‘钢琴必须追求超高难度炫技才有意义’这一教条的最好反驳。其妙处在于克制：同样的四个和弦循环，仅凭微弱的触键强弱与音色明暗变化，便能唤起深邃的宁静与浩渺。"
      },
      highlights: {
        en: "Nuvole Bianche, Divenire, Una Mattina, Experience, I Giorni, and live acoustic performances.",
        zh: "《白云》（Nuvole Bianche）、《演化》（Divenire）、《初阳》（Una Mattina）、《历程》（Experience）、《那些日子》（I Giorni）以及原声现场录音。"
      },
      takeaway: {
        en: "Touch, space, and tonal color matter infinitely more than mechanical velocity. The silence between notes is music too.",
        zh: "音色触键与留白远比机械的手指跑动速度重要。音符之间的呼吸与静默，同样是最高级的音乐。"
      }
    },
    {
      id: "passacaglia-masterworks",
      name: "Handel-Halvorsen: Passacaglia Interpretations",
      channel: "Baroque Virtuosity & Architecture",
      role: {
        en: "The Summit of Classical Variation & Drama",
        zh: "巴洛克建筑之美与变奏狂澜的永恒巅峰"
      },
      tags: ["Passacaglia", "Chaconne", "Monumental Drama", "Personal Summit"],
      link: "https://www.youtube.com/results?search_query=Passacaglia+Handel+Halvorsen+piano",
      description: {
        en: "From Johan Halvorsen's fiery violin/viola arrangement of Handel's Harpsichord Suite in G minor (HWV 432) to virtuosic solo piano adaptations.",
        zh: "从哈尔沃森对亨德尔 G小调大键琴组曲（HWV 432）变奏的狂热提琴二重奏改编，到各种令琴键燃烧的钢琴独奏版本。"
      },
      whyIEnjoy: {
        en: "This piece represents the grand architectural peak of my own adult piano journey. Listening to diverse master performances continuously recharges my determination to conquer polyphony and multi-layered voicings.",
        zh: "这首作品是我成年学琴历程中攀登的最巍峨的峰峦。聆听不同大师与新锐演奏家的现场演绎，每一次都能彻底刷新我对复调交响感、变奏推进张力与巴洛克骨骼美感的认知。"
      },
      highlights: {
        en: "Dramatic 8-bar descending ground bass; explosive sixteenth-note arpeggiated variations; thrilling orchestral climaxes.",
        zh: "庄严肃穆的8小节固定低音恰空骨架；排山倒海的十六分音符奔流变奏；波澜壮阔的交响化和弦扫击。"
      },
      takeaway: {
        en: "Baroque counterpoint teaches discipline; romantic passion gives it fire. An adult can tackle monuments by mastering the underlying architectural laws.",
        zh: "巴洛克对位赋予我们严谨与纪律，浪漫情怀赋予我们烈火。成年人通过领悟底层的建筑法则，完全能够亲手筑起心中的古典丰碑。"
      }
    },
    {
      id: "nahre-sol",
      name: "Nahre Sol",
      channel: "@NahreSol",
      role: {
        en: "Composer, Pianist & Creative Practice Explorer",
        zh: "作曲家、钢琴家与充满探索趣味的音乐实验家"
      },
      tags: ["Curiosity", "Harmonic Playground", "Genre Blending", "Mindset"],
      link: "https://www.youtube.com/@NahreSol",
      description: {
        en: "Juilliard graduate whose visual practice journals and style deconstructions turn practice into a joyful, curious laboratory.",
        zh: "毕业于茱莉亚音乐学院的先锋创作者，其练习日志与风格解构将练琴还原为充满好奇与审美愉悦的音乐实验室。"
      },
      whyIEnjoy: {
        en: "Her videos remind me that we play piano to *play*—to explore harmony, experiment with textures, and cultivate genuine musical taste rather than mindlessly checking off etudes.",
        zh: "她的作品总在提醒我：弹琴（Play）的本意是‘玩’与‘探索’——去好奇不同和弦的色彩，去尝试不同触键织体，去培育真正的音乐审美，而不是死板地完成考级打卡。"
      },
      highlights: {
        en: "'How to Sound Like...' series, practice diary deep-dives, classical-to-jazz harmonic conversions.",
        zh: "《如何弹得像XX风格》系列、真实练琴日记解构、古典作品到爵士和声色彩的即兴演变。"
      },
      takeaway: {
        en: "Cultivate curiosity over mechanical duty. When practice is an adventure in discovery, consistency takes care of itself.",
        zh: "以探索的好奇心取代苦行僧式的机械义务。当练琴成为一场每天都有新发现的冒险，自律便不再需要强撑。"
      }
    }
  ]
};
